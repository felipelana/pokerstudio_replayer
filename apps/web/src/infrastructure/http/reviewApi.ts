...
export const reviewApi = {
  list: () =>
    api.get<{ items: (CloudReviewRow & { _count: { hands: number } })[]; quota: { perDay: number; usedToday: number } }>(
      '/reviews',
    ),
  get: (id: string) => api.get<CloudReviewRow & { hands: (CloudReviewHand & { notes: CloudReviewNote[] })[] }>(`/reviews/${id}`),
  save: (id: string, review: CloudReviewInput) => api.put<{ id: string; created: boolean }>(`/reviews/${id}`, review),
  progress: (id: string, body: { currentHandIndex: number; currentActionIndex?: number; status?: 'IN_PROGRESS' | 'COMPLETED' }) =>

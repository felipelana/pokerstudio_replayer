...
import { adminApi, type ErrorLevel, type ErrorSource } from '@/infrastructure/http/adminApi';
    ...
    <div className="flex flex-col gap-3">
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        {(['FATAL', 'ERROR', 'WARN'] as ErrorLevel[]).map((value) => (
          <div key={value} className="panel p-3">
            ...
            <div className="mt-1 text-2xl font-semibold tabular-nums" style={{ color: levelColour(value) }}>
      ...
      <div className="flex flex-wrap items-end gap-2">
        <label className="flex flex-col text-sm">
          ...
          <select className="input w-[140px]" value={level} onChange={(e) => change(() => setLevel(e.target.value))}>
        ...
        <label className="flex flex-col text-sm">
          ...
          <select className="input w-[140px]" value={source} onChange={(e) => change(() => setSource(e.target.value))}>
            <option value="">{t('admstudio.any')}</option>
            {SOURCES.map((value) => (
              <option key={value} value={value}>
          ...
          <tbody>
            {(data?.items ?? []).map((row) => (
              <tr key={row.id} style={{ borderTop: '1px solid var(--border)' }}>
                <td className="p-2">
                  <span className="chip-tag" style={{ color: levelColour(row.level), borderColor: levelColour(row.level) }}>
                    {t(`admstudio.levels.${row.level}`)}
                  </span>
                </td>
                <td className="whitespace-nowrap p-2 tabular-nums" style={{ color: 'var(--text-muted)' }}>
      ...
      {data && (
        <>
          ...
          {data.context && Object.keys(data.context).length > 0 && (
            <pre className="max-h-[160px] overflow-auto rounded p-2 font-mono text-[11px]" style={{ background: 'var(--bg-elevated, var(--bg))' }}>
              {JSON.stringify(data.context, null, 2)}
            </pre>
          )}

          <pre className="max-h-[320px] overflow-auto rounded p-2 font-mono text-[11px]" style={{ background: 'var(--bg-elevated, var(--bg))' }}>

...
import { coachLink, shareApi, type CoachAssessment, type Invite, type NewInvite } from '@/infrastructure/http/shareApi';
import { ApiError } from '@/infrastructure/http/client';
import { IconCheck, IconClose, IconCopy, IconEye, IconRefresh, IconTrash } from '../icons';
...
export function ShareReviewDialog({
  ...
  const stateOf = (invite: Invite) => {
    ...
    if (new Date(invite.expiresAt) <= new Date()) return { label: t('share.stateExpired'), color: 'var(--text-muted)' };
    if (invite.assessment?.completedAt) return { label: t('share.stateDone'), color: 'var(--result-won)' };
    if (invite.assessment) return { label: t('share.stateReading'), color: 'var(--result-break-even)' };
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 pb-5">
            ...
            >
              ...
              <label className="flex flex-col gap-1 text-sm">
                ...
                <span className="flex gap-2">
                  ...
                  />
                  <button type="button" className="btn" onClick={() => setPassword(freshPassword())} title={t('share.newPassword')}>
                    <IconRefresh size={14} />
                    {t('share.newPassword')}
                  </button>
                </span>
                <span className="text-xs leading-relaxed" style={{ color: problems.length ? 'var(--result-lost)' : 'var(--text-muted)' }}>
          ...
          {invites.length > 0 && (
            <section className="mt-6">
                    ...
                    <li key={invite.id} className="flex items-center gap-2 text-xs">
                      ...
                      {invite.assessment && (
                        <button
                          ...
                          onClick={() => setReading(reading?.id === invite.id ? undefined : { id: invite.id, assessmentId: invite.assessment!.id, coachName: invite.coachName })}
                        >
                          <IconEye size={13} />
                        </button>
                      )}
              ...
              {reading && <CoachFeedback assessmentId={reading.assessmentId} coachName={reading.coachName} />}
            </section>
          )}
        </div>
      </div>
...
function CoachFeedback({ assessmentId, coachName }: { assessmentId: string; coachName: string }) {
  ...
  if (failed) return <p className="mt-3 text-xs" style={{ color: 'var(--result-lost)' }}>{t('auth.offline')}</p>;
  if (!data) return <p className="mt-3 text-xs" style={{ color: 'var(--text-muted)' }}>{t('common.loading')}</p>;

  const written = data.rows.filter((row) => row.assessment && (row.assessment.comment || typeof row.assessment.score === 'number' || row.assessment.markedOk));
  ...
  return (
    <div className="mt-3 rounded-lg border p-3" style={{ borderColor: 'var(--border)' }}>
      ...
      {written.length === 0 ? (
        <p className="text-xs" style={{ color: 'var(--text-muted)' }}>{t('share.feedbackEmpty')}</p>
      ) : (
        <ul className="flex flex-col gap-2">
          {written.map((row) => (
            <li key={row.index} className="text-xs">
              <span className="font-semibold">{t('share.handNumber', { number: row.index + 1 })}</span>
              {typeof row.assessment?.score === 'number' && (
                <span className="ml-2 tabular-nums" style={{ color: 'var(--text-muted)' }}>{row.assessment.score}/100</span>
              )}
              {row.assessment?.markedOk && <IconCheck size={12} />}
              {row.assessment?.comment && (
                <p className="mt-0.5 whitespace-pre-wrap leading-relaxed" style={{ color: 'var(--text-muted)' }}>
                  {row.assessment.comment}
                </p>
              )}
            </li>

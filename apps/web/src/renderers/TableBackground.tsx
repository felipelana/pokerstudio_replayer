...
import type { BackgroundSkin, UiSkin } from '@/skins/types';
...
export function gradientCss(g: NonNullable<BackgroundSkin['gradient']>): string {
  const stops = [...g.stops].sort((a, b) => a.at - b.at).map((s) => `${s.color} ${Math.round(s.at * 100)}%`).join(', ');
  return g.type === 'radial' ? `radial-gradient(circle at 50% 45%, ${stops})` : `linear-gradient(${g.angle}deg, ${stops})`;
}
  ...
  if (!bg) {
    return (
      <div
        ...
        style={{ background: `linear-gradient(180deg, ${ui.bg}, ${ui.bgEnd})`, pointerEvents: 'none', zIndex: 0 }}
      />
    );
  }

  return (
    <div aria-hidden="true" className="absolute inset-0" style={{ pointerEvents: 'none', zIndex: 0 }}>

...
import { buildReplay, potOddsFor, quickResult } from '@/engine/replay';
import type { Hand } from '@/model/types';

const parser = new PokerStarsParser();
const text = readFileSync(join(__dirname, '..', '..', '..', 'samples', 'pokerstars-demo.txt'), 'utf8');
...
describe('samples/pokerstars-demo.txt', () => {
  ...
  it('pot odds match the manual calculation on 5 hands', () => {
    ...
    const f1 = h1.frames.find((f) => f.action?.type === 'raise' && f.action.player === 'villain one')!;
    ...
    for (const f of h8.frames) if (f.action?.player === 'Hero') expect(potOddsFor(f, 'Hero')).toBeUndefined();

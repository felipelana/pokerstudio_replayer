...
import { AVAILABLE_ROOMS, COMING_SOON_ROOMS, isRoomAvailable, roomForSite, roomName, ROOMS } from '@pokerstudio/shared';
...
describe('the room catalogue', () => {
  ...
  it('marks every room the importer cannot read as coming soon', () => {
    for (const room of COMING_SOON_ROOMS) {
      ...
      expect(() => parser.parse('not a hand'), `${room.name} claims coming-soon but parses`).toThrow();
    }
  });
  ...
  it('includes the rooms the settings screen has to offer a nickname for', () => {
    ...
    for (const expected of ['pokerstars', 'chico', 'ggpoker', '888poker', 'ipoker', 'wpn', 'coinpoker', 'ignition']) {

...
export async function saveReview(
  ...
  const existing = await deps.prisma.reviewSession.findUnique({ where: { id: review.id }, select: { userId: true } });
  ...
  await deps.prisma.$transaction(async (tx) => {
    if (existing) {
      ...
      await tx.reviewSession.update({ where: { id: review.id }, data: { ...header, hands: { create: hands } } });
    } else {
      await tx.reviewSession.create({ data: { id: review.id, userId, ...header, hands: { create: hands } } });
    }
    await tx.usageEvent.create({
      data: { userId, type: existing ? 'REVIEW_SAVE' : 'REVIEW_START', meta: { handCount: review.handCount } },
    });
  });
...
export async function saveProgress(
  ...
  if (input.status === 'COMPLETED') {
    await deps.prisma.usageEvent.create({ data: { userId: input.userId, type: 'REVIEW_COMPLETE' } });
  }
  return ok({ ok: true });
}

export async function deleteReview(deps: ReviewDeps, userId: string, id: string): Promise<Result<{ ok: true }>> {

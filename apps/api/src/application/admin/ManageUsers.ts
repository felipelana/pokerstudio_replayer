...
import type { AccessLogRepository, SessionRepository, UserRepository } from '../../domain/repositories/index.js';
...
export async function unblockUser(deps: AdminDeps, input: { adminId: string; userId: string }): Promise<Result<User>> {
  const target = await deps.users.findById(input.userId);
  if (!target) return fail(Errors.notFound('User'));
  const updated = await deps.users.update(target.id, { status: 'ACTIVE', blockedReason: undefined });
  await deps.log.record({ userId: target.id, email: target.email, event: 'ADMIN_UNBLOCK', detail: { by: input.adminId } });
  return ok(updated);
}
...
export async function revokeUserSessions(
  ...
  if (!target) return fail(Errors.notFound('User'));
  const revoked = await deps.sessions.revokeAllForUser(target.id);
  await deps.log.record({ userId: target.id, event: 'SESSION_REVOKE', detail: { by: input.adminId, revoked } });
  return ok({ revoked });
}

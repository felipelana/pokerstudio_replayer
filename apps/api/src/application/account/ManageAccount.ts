...
export async function updateProfile(
  ...
  if (input.patch.name !== undefined) {
    ...
    if (name.length < 2) return fail(appError('invalid_name', 'Tell us a name with at least 2 characters.', 422));
    patch.name = name;
  }
...
export async function changePassword(
  deps: AccountDeps,
  input: { user: User; currentPassword: string; newPassword: string; keepSessionId: string; ip?: string },
): Promise<Result<{ revoked: number }>> {
  // An account that only signs in with Google has no password to check against.
  if (!input.user.passwordHash) return fail(appError('password_not_set', 'This account has no password yet.', 409));
  if (!(await deps.hasher.verify(input.user.passwordHash, input.currentPassword))) {
    await deps.log.record({ userId: input.user.id, event: 'LOGIN_FAIL', ip: input.ip, detail: { at: 'change_password' } });
  ...
  if (await deps.breach.isBreached(password.value.value)) {
    return fail(appError('breached_password', 'This password appears in known leaks. Choose another.', 422));
  }

  await deps.users.update(input.user.id, { passwordHash: await deps.hasher.hash(password.value.value) });
  ...
  await deps.log.record({ userId: input.user.id, event: 'RESET_OK', ip: input.ip, detail: { method: 'CHANGE_PASSWORD' } });
  return ok({ revoked });
}
...
export async function setFirstPassword(
  ...
  if (input.user.passwordHash) return fail(appError('password_already_set', 'This account already has a password.', 409));
  ...
  await deps.users.update(input.user.id, { passwordHash: await deps.hasher.hash(password.value.value) });
  await deps.log.record({ userId: input.user.id, event: 'RESET_OK', ip: input.ip, detail: { method: 'SET_PASSWORD' } });
  return ok({ ok: true });
}
 ...
 */
export async function exportAccount(deps: AccountDeps, user: User): Promise<Result<Record<string, unknown>>> {
  ...
  return ok({
    ...
    skins: skins.map((s) => ({ skinId: s.skinId, name: s.name, updatedAt: s.updatedAt, data: s.data })),
    referrals,
...
export async function deleteAccount(
  ...
  await deps.log.record({ userId: input.user.id, event: 'ACCOUNT_DELETED', ip: input.ip, detail: { at: now.toISOString() } });
  return ok({ ok: true });
}

...
import type { UserRepository, EmailTokenRepository, AccessLogRepository, ReferralRepository } from '../../domain/repositories/index.js';
 ...
 */
export async function signUpUser(deps: SignUpDeps, input: SignUpInput): Promise<Result<{ user: User; verificationToken?: string }>> {
  ...
  if (!/^[A-Za-z]{2}$/.test(input.countryCode)) return fail(appError('invalid_country', 'Choose your country.', 422));
  ...
  const referrer = input.referralCode ? await deps.users.findByReferralCode(input.referralCode.toUpperCase()) : undefined;
  ...
  await deps.email.send({ to: user.email, template: 'verify-email', locale: user.language, payload: { name: user.name, token } });
  if (referrer) await deps.referrals.markAccepted(input.referralCode!.toUpperCase(), user.id, now);
  await deps.log.record({ userId: user.id, email: user.email, event: 'SIGNUP', ip: input.ip, userAgent: input.userAgent });

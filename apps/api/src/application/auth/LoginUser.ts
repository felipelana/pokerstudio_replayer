...
import type { Clock, GeoIpResolver, Hasher, TokenGenerator, UserAgentParser } from '../ports/index.js';
...
export async function loginUser(
  ...
  const passwordOk = user?.passwordHash ? await deps.hasher.verify(user.passwordHash, input.password) : false;

  if (!user || !passwordOk || !canAuthenticate(user, deps.requireVerification)) {
    for (const key of keys) await deps.attempts.registerFailure(key, now, MAX_ATTEMPTS, WINDOW_MS, LOCK_MS);
    await recordFailure(deps, {
      email,
      input,
      event: user && !canAuthenticate(user, deps.requireVerification) ? 'BLOCKED_ATTEMPT' : 'LOGIN_FAIL',

...
import type { AccessLogRepository, EmailTokenRepository, UserRepository } from '../../domain/repositories/index.js';
...
export async function verifyEmail(deps: VerifyEmailDeps, token: string, ip?: string): Promise<Result<User>> {
  ...
  if (user.status === 'BLOCKED' || user.status === 'DELETED') return fail(Errors.accountUnavailable());

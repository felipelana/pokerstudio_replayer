...
export function createGoogleProvider(config: GoogleConfig): OAuthProvider {
  let jwksCache: { keys: (JsonWebKey & { kid: string; alg?: string })[]; fetchedAt: number } | undefined;
...
export async function verifyIdToken(
  ...
  keyFor: (kid: string) => Promise<Parameters<typeof createVerify>[0] extends never ? never : import('node:crypto').KeyObject>,
  now: number = Date.now(),

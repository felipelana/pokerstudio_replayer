...
export class Password {
  ...
  static create(raw: string): Result<Password> {
    if (raw.length < MIN_PASSWORD_LENGTH) {
      return fail(appError('weak_password', `Use at least ${MIN_PASSWORD_LENGTH} characters.`, 422));
    }
    if (raw.length > 256) return fail(appError('weak_password', 'The password is too long.', 422));
    if (/^(.)\1+$/.test(raw)) return fail(appError('weak_password', 'The password is too simple.', 422));

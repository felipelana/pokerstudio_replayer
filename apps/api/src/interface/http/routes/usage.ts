...
const USAGE_TYPES = ['APP_OPEN', 'HAND_IMPORT', 'REVIEW_START', 'REPORT_EXPORT', 'SKIN_APPLIED'] as const;
...
export async function usageRoutes(app: FastifyInstance, container: AppContainer) {
  app.post('/usage', { config: { rateLimit: { max: 120, timeWindow: '1 minute' } } }, async (request, reply) => {
    const body = z
      .object({
        type: z.enum(USAGE_TYPES),
        anonId: z.string().min(8).max(64).regex(/^[A-Za-z0-9_-]+$/).optional(),

...
import { z } from 'zod';
import { deleteUserSkin, listUserSkins, saveUserSkin } from '../../../application/skins/UserSkins.js';
...
export async function skinRoutes(app: FastifyInstance, container: AppContainer) {
  app.get('/skins', async (request, reply) => {
    ...
    return reply.send(
      result.value.map((s) => ({ id: s.skinId, name: s.name, data: s.data, updatedAt: s.updatedAt.toISOString() })),
    );
  });

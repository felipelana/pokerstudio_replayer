...
import { z } from 'zod';
import { disableTotp, startTotpEnrollment, useRecoveryCode, verifyTotp } from '../../../application/auth/TwoFactor.js';
...
export async function twoFactorRoutes(app: FastifyInstance, container: AppContainer) {
  ...
  const requireAdminRole = (request: Parameters<typeof requireUser>[0], reply: Parameters<typeof requireUser>[1]) => {
  ...
  app.get('/admin/2fa', async (request, reply) => {
    ...
    return reply.send({
      ...
      recoveryCodesLeft: enrollment?.confirmedAt ? await container.twoFactor.countRecoveryCodes(user.id) : 0,
    });
  });
  ...
  app.post('/admin/2fa/verify', async (request, reply) => {
    ...
    const result = await verifyTotp(deps, { user, sessionId: request.currentSessionId!, code, ip: clientIp(request) });
    if (!result.ok) return problem(reply, result.error);
    return reply.send(result.value);
  });
  ...
  app.post('/admin/2fa/recovery', async (request, reply) => {
    ...
    const result = await useRecoveryCode(deps, { user, sessionId: request.currentSessionId!, code, ip: clientIp(request) });
    if (!result.ok) return problem(reply, result.error);
    return reply.send(result.value);
  });

...
import { REDACTED, normaliseRoute, recordError, redact } from '../src/application/observability/RecordError.js';
...
describe('route normalisation', () => {
  it('turns the ids of one request into the shape of many', () => {
    expect(normaliseRoute('get', '/api/v1/reviews/3f2504e0-4f89-11d3-9a0c-0305e82c3301/hands/1204?x=1')).toBe(
      'GET /api/v1/reviews/:id/hands/:id',
    );
  });
...
describe('recordError', () => {
  it('stamps the environment and the release on every row', async () => {
    ...
    await recordError({ errors, env: 'staging', release: '1.2.3' }, { source: 'SERVER', message: 'boom' });

    expect(errors.entries).toHaveLength(1);
    expect(errors.entries[0]).toMatchObject({ env: 'staging', release: '1.2.3', level: 'ERROR', message: 'boom' });
  });
  ...
  it('never throws when the store itself is broken', async () => {
    ...
    await expect(
      recordError({ errors: broken, env: 'production', onFailure: (e) => (reported = e) }, { source: 'CLIENT', message: 'x' }),

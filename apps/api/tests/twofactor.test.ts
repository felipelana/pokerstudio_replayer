...
import { disableTotp, startTotpEnrollment, useRecoveryCode, verifyTotp } from '../src/application/auth/TwoFactor.js';
import { createCipher, totpProvider } from '../src/infrastructure/crypto/index.js';
import type { TwoFactorEnrollment, TwoFactorRepository } from '../src/domain/repositories/index.js';
...
function fakeTwoFactor(): TwoFactorRepository & { row?: TwoFactorEnrollment; codes: { hash: string; usedAt?: Date }[] } {
  const store: { row?: TwoFactorEnrollment; codes: { hash: string; usedAt?: Date }[] } = { codes: [] };
  return {
    ...store,
    get row() {
      return store.row;
...
describe('two-factor enrolment', () => {
  ...
  it('refuses to verify when nothing was ever enrolled', async () => {
    const d = deps();
    const result = await verifyTotp(d, { user: admin, sessionId: await session(d), code: '123456' });
    expect(result.ok).toBe(false);
...
describe('recovery codes', () => {
  it('lets one code through exactly once', async () => {
    const d = deps();
    await startTotpEnrollment(d, admin);
    const enrol = await verifyTotp(d, { user: admin, sessionId: await session(d), code: currentCode(d) });
  ...
  it('accepts a code typed in lower case with spaces around it', async () => {
    ...
    const used = await useRecoveryCode(d, { user: admin, sessionId: await session(d), code: messy });
    expect(used.ok).toBe(true);
  });
});

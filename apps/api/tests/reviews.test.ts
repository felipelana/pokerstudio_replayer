...
import {
  ...
  saveReview,
  uploadsToday,
} from '../src/application/reviews/CloudReviews.js';
...
const review = (over: Record<string, unknown> = {}) => ({
  ...
  hands: [
    ...
    { index: 1, handId: '2500000002', rawHistory: 'PokerStars Hand #2500000002', result: 'FOLDED' as const },
  ],
  ...over,
});
...
describe('reviews in the cloud', () => {
  ...
  it('stops at the daily upload limit but still accepts updates', async () => {
    ...
    expect((await saveReview(deps, { userId, review: { ...ids[0], title: 'Renamed' } })).ok).toBe(true);
  });
  ...
  it('records progress and marks the review complete', async () => {
    ...
    expect((await saveProgress(deps, { userId, id: input.id, currentHandIndex: 1, currentActionIndex: 4 })).ok).toBe(true);
  ...
  it('never shows or touches another account’s review', async () => {
    ...
    expect((await saveProgress(deps, { userId: other.id, id: input.id, currentHandIndex: 9 })).ok).toBe(false);
    expect((await deleteReview(deps, other.id, input.id)).ok).toBe(false);

    const stolen = await saveReview(deps, { userId: other.id, review: { ...input, title: 'Mine now' } });

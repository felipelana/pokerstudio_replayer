...
import { tokenGenerator } from '../src/infrastructure/crypto/index.js';
...
const player = await signedInUser('player');
  ...
  const session = await prisma.reviewSession.create({ data: { userId: player.id, title: 'Sunday Storm', handCount: 3 } });
  reviewId = session.id;
  ...
  const own = await app.inject({ method: 'GET', url: `/api/v1/reviews/${reviewId}/assessment`, headers: { cookie: playerCookie } });
...
describe('the coach gets one review and nothing else', () => {
  ...
  it('cannot touch the player self-assessment, even knowing its id', async () => {
    ...
    const detail = await app.inject({ method: 'GET', url: `/api/v1/assessments/${selfAssessmentId}`, headers: { cookie: playerCookie } });
  ...
  it('never sees another coach, and the player sees both', async () => {
    ...
    const anaSees = JSON.parse((await app.inject({ method: 'GET', url: '/api/v1/coach/assessment', headers: { cookie: ana.cookie } })).body);
    ...
    expect(anaSees.rows.every((r: { assessment: { score: number } | null }) => r.assessment === null || r.assessment.score === 20)).toBe(true);
    expect(anaSees.summary.score).toEqual({ value: 20, scored: 1 });
    expect(JSON.stringify(anaSees)).not.toContain('Bruno');

    const player = JSON.parse(
      (await app.inject({ method: 'GET', url: `/api/v1/reviews/${reviewId}/assessments`, headers: { cookie: playerCookie } })).body,
    );
    expect(player.items).toHaveLength(3);
    expect(player.items.map((i: { summary: { score: { value: number | null } } }) => i.summary.score.value).sort()).toEqual([20, 90, null]);
  });
  ...
  it('lets the player read a coach but never rewrite one', async () => {
    ...
    const read = await app.inject({ method: 'GET', url: `/api/v1/assessments/${coachAssessmentId}`, headers: { cookie: playerCookie } });
    ...
    expect(JSON.parse((await app.inject({ method: 'GET', url: `/api/v1/assessments/${coachAssessmentId}`, headers: { cookie: playerCookie } })).body).rows[0].assessment.score).toBe(33);
  });

  it('keeps a stranger out of the review entirely', async () => {
    const res = await app.inject({ method: 'GET', url: `/api/v1/reviews/${reviewId}/assessments`, headers: { cookie: strangerCookie } });
...
describe('when the link stops being valid', () => {
  it('shuts a browser that is already open, on the very next request', async () => {
    ...
    expect((await app.inject({ method: 'GET', url: '/api/v1/coach/session', headers: { cookie } })).statusCode).toBe(200);

    await prisma.coachInvite.update({ where: { id: invite.id }, data: { expiresAt: new Date(Date.now() - 1000) } });

    const after = await app.inject({ method: 'GET', url: '/api/v1/coach/session', headers: { cookie } });
  ...
  it('closes at once when revoked, and keeps every word already written', async () => {
    ...
    const after = await app.inject({ method: 'GET', url: '/api/v1/coach/assessment', headers: { cookie } });
    expect(after.statusCode).toBe(403);
...
describe('finishing, over HTTP', () => {
  it('locks the coach out of their own reading, with the link still live', async () => {
    ...
    expect((await app.inject({ method: 'POST', url: '/api/v1/coach/assessment/complete', ...sending(cookie) })).statusCode).toBe(200);
  ...
  it('does not finish anyone else when one assessor finishes', async () => {
    const ana = await inviteAndOpen('Ana');
    await app.inject({ method: 'POST', url: '/api/v1/coach/assessment/complete', ...sending(ana.cookie) });

 ...
 * `apps/api`, and this file is only the doorway between a web Request and them.
 *
...
/** The API's own .env, read once, so there is one file and not two. */
function loadApiEnv() {
  if (process.env.DATABASE_URL) return;
  for (const name of ['.env', '.env.local']) {
    try {
      const text = readFileSync(path.join(API_ROOT, name), 'utf8');
...
async function api(): Promise<FastifyInstance> {
  if (!globalThis.__pokerstudioApi) {
    globalThis.__pokerstudioApi = (async () => {
      loadApiEnv();
      const { loadConfig } = await import('@pokerstudio/api/shared/config.js');
      const { createContainer } = await import('@pokerstudio/api/main-container.js');
      const { buildServer } = await import('@pokerstudio/api/interface/http/server.js');

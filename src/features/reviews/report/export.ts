...
function summaryLines(report: ReportData, labels: ReportLabels): string[] {
  ...
  if (report.summary.leaks.length) {
    lines.push(
      `${labels.leaks}: ${report.summary.leaks
        .slice(0, 8)
        .map((l) => `${l.tag} (${l.hands})`)
        .join(', ')}`,
    );
  ...
  for (const coach of report.coaches) {
    lines.push(
      `${coach.name}: ${formatScore(coach.summary.score.value) ?? labels.noScore} · ${labels.coverage} ${pct(coach.summary.coverage.percent)}`,
    );
  }
  return lines;
}
...
export function exportPdf(report: ReportData, labels: ReportLabels) {
  ...
  for (const item of report.items) {
    ...
    if (item.tags.length)
      line(`${labels.tags}: ${item.tags.map((t) => t.label).join(', ')}`, 9, false, 2);
    const own = scoreOf(item.review);
    const stars = item.review.rating ? ` ${'★'.repeat(item.review.rating)}` : '';
    if (own !== undefined) line(`${labels.rating}: ${own}/100${stars}`, 9, false, 2);
    if (item.image) {
    ...
    for (const c of item.coaches) {
      const head = [
        `${labels.coach} · ${c.name}`,
        typeof c.reading.score === 'number' ? `${c.reading.score}/100` : '',
      ]
        .filter(Boolean)
        .join(': ');
      line(head, 9, true, 2);
...
export async function exportDocx(report: ReportData, labels: ReportLabels) {
  const children: Paragraph[] = [
    ...
    new Paragraph({
      children: [
        new TextRun({ text: report.generatedAt.toLocaleString(), size: 18, color: '666666' }),
      ],
    }),
    new Paragraph({ text: labels.summary, heading: HeadingLevel.HEADING_2 }),
    ...summaryLines(report, labels).map(
      (text) => new Paragraph({ children: [new TextRun({ text, size: 18 })] }),
    ),
  ];
  ...
  for (const item of report.items) {
    ...
    if (item.tags.length) {
      children.push(
        new Paragraph({
          children: [
            new TextRun({
              text: `${labels.tags}: ${item.tags.map((t) => t.label).join(', ')}`,
              italics: true,
              size: 18,
            }),
          ],
    ...
    if (own !== undefined) {
      ...
      children.push(
        new Paragraph({
          children: [new TextRun({ text: `${labels.rating}: ${own}/100${stars}`, size: 18 })],
        }),
      );
    }
    if (item.image) {
      ...
      children.push(
        new Paragraph({
          ...
          children: [
            new ImageRun({ data, transformation: { width: 520, height: 333 }, type: 'png' }),
          ],
        }),
      );
    }
    ...
    for (const c of item.coaches) {
      const head = [
        ...
        .join(': ');
      children.push(
        new Paragraph({ children: [new TextRun({ text: head, bold: true, size: 18 })] }),
      );
      if (c.reading.comment)
        children.push(
          new Paragraph({ children: [new TextRun({ text: c.reading.comment, size: 18 })] }),
        );
    }
    for (const [street, note] of Object.entries(item.review.streetNotes ?? {})) {
      if (note?.trim())
        children.push(
          new Paragraph({
            children: [new TextRun({ text: `${street}: ${note.trim()}`, size: 18 })],
          }),
        );
    }
  }

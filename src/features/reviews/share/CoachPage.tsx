...
import { shareApi, type CoachSession, type HandReading } from '@/lib/infrastructure/http/shareApi';
import { ApiError } from '@/lib/infrastructure/http/client';
import { LanguageSelector } from '@/components/ui/LanguageSelector';
import { IconCheck, IconEye, IconLogout } from '@/components/ui/icons';
import { useDateFormatter } from '@/components/hooks/useFormat';
import { useAppStore } from '@/lib/state/store';
import { buildReplay } from '@/domain/engine/replay';
import { parseInWorker } from '@/features/parsers/importer';
import { TableArea } from '@/features/replayer/ui/TableArea';
import type { Session } from '@/domain/model/types';
import brandMark from '@/lib/assets/pokerstudio-mark.png';
...
export function CoachPage() {
  ...
  const open = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        problem ? (problem.status === 422 ? t('coach.badLink') : problem.title) : t('auth.offline'),
      );
    } finally {
      setBusy(false);
    }
...
function CoachReview({ session }: { session: CoachSession }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [payload, mine] = await Promise.all([
          shareApi.hands(),
          shareApi.reading().catch(() => undefined),
        ]);
        if (!alive) return;
        if (mine) {
        ...
        const result = await parseInWorker(
          payload.items.map((item) => item.rawHistory).join('\n\n'),
        );
        if (!alive) return;
    ...
    return (
      <main className="min-h-0 flex-1 overflow-auto p-6">
        <div className="mx-auto max-w-[760px]">
          ...
          <p
            className="mt-6 rounded-lg border border-dashed p-4 text-sm leading-relaxed"
            style={{ borderColor: 'var(--border)', color: 'var(--text-muted)' }}
          >
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-y-auto lg:flex-row lg:overflow-hidden">
      ...
      <main className="flex min-h-[420px] min-w-0 shrink-0 flex-col lg:min-h-0 lg:flex-1 lg:shrink">
        ...
        <div
          className="flex items-center justify-center gap-2 border-t p-2 text-xs"
          style={{ borderColor: 'var(--border)' }}
        >
          <button
            type="button"
            className="btn"
            onClick={() => selectHand(handIndex - 1)}
          ...
          <button
            type="button"
            className="btn"
            onClick={() => setFrame(Math.max(0, frameIndex - 1))}
          ...
          <button
            ...
            className="btn"
            onClick={() => selectHand(handIndex + 1)}
            disabled={handIndex >= hands.length - 1}
          >
            {t('coach.nextHand')}
          </button>
        </div>
      ...
      <aside
        className="shrink-0 border-t p-3 lg:w-80 lg:overflow-auto lg:border-l lg:border-t-0"
        style={{ borderColor: 'var(--border)' }}
      >
        <ReadingPanel
          key={reviewIndex}
...
function ReadingPanel({
  ...
  const [score, setScore] = useState(
    typeof reading.score === 'number' ? String(reading.score) : '',
  );
  return (
    <div className="flex flex-col gap-3">
      ...
      <label className="flex flex-col gap-1 text-sm">
        ...
        <input
          ...
          onBlur={() =>
            onSave({ score: score === '' ? null : Math.max(0, Math.min(100, Number(score))) })
          }
        />
      </label>

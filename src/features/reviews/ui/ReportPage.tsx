...
import { buildReport, handHeadline, type ReportData } from '@/features/reviews/report/buildReport';
import type { UserTag } from '@/domain/model/types';
import { exportDocx, exportPdf } from '@/features/reviews/report/export';
import { leakLabel, scoreOf } from '@/features/reviews/report/assessment';
          ...
          <article key={item.hand.id} className="panel flex flex-col gap-2 p-4">
            ...
            {item.tags.length > 0 && (
              <div className="flex flex-wrap gap-1">
                {item.tags.map((tag) => (
                  <span
                    key={tag.id}
                    className="rounded-full px-2 py-[2px] text-[11px] text-white"
            ...
            {scoreOf(item.review) !== undefined && (
              <div className="flex items-center gap-2 text-sm">
                ...
                {item.review.rating && (
                  <span style={{ color: 'var(--result-break-even)' }}>
            ...
            {item.review.notes.trim() && (
              <p className="whitespace-pre-wrap text-sm">{item.review.notes}</p>
            )}
            {item.coaches.map((c) => (
              <div
                key={c.name}
              ...
              >
                ...
                {c.reading.comment && (
                  <p className="mt-0.5 whitespace-pre-wrap">{c.reading.comment}</p>
                )}
              </div>
            ))}
...
function Numbers({ report, tags }: { report: ReportData; tags: UserTag[] }) {
  ...
  const card = (title: string, value: string, detail: string, accent = false) => (
  ...
  const pct = (value: number | null) =>
    value === null ? t('report.noCoverage') : `${Math.round(value)}%`;
    ...
    <section className="flex flex-col gap-3" aria-label={t('report.summary')}>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        {card(
          ...
          t('report.coverageDetail', {
            reviewed: report.summary.coverage.reviewed,
            total: report.summary.coverage.total,
          }),
        )}
        {card(
          t('report.reviewed'),
          String(report.summary.reviewedHands),
          t('report.reviewedDetail'),
        )}
      </div>

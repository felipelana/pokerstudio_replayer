...
describe('Chico: telling it apart from PokerStars', () => {
  ...
  it('leaves a genuine PokerStars file to PokerStars', () => {
    const stars = readFileSync(
      join(__dirname, '..', 'pokerstars', 'fixtures', 'real-ko-tournament-dealer-export.txt'),
      'utf8',
    );
...
describe('Chico: the 660 real hands', () => {
  ...
  it('parses all of them without a single warning, from the parser or the engine', () => {
    ...
    for (const hand of all) {
      ...
      for (const w of replay.warnings)
        if (!hand.warnings.includes(w)) problems.push(`#${hand.handNumber} engine: ${w}`);
    }
  ...
  it('balances every pot: what went in, less what came back, is what was declared', () => {
    const off = all
      .map((hand) => ({
        ...
        put: contributed(hand),
        declared: Math.round(hand.summary.totalPot),
      }))
  ...
  it('pays out exactly what the pot held, since the rake is zero in these tournaments', () => {
    const off = all
      .map((hand) => {
        const won = hand.summary.pots
          .flatMap((p) => p.winners)
          .reduce((sum, w) => sum + w.amount, 0);
        return {
          hand: hand.handNumber,
          won: Math.round(won),
          pot: Math.round(hand.summary.totalPot - hand.summary.rake),
        };
      })
  ...
  it('reads a showdown, its board and the winner', () => {
    const showdown = pko.find(
      (h) => h.showdown.some((a) => a.type === 'show') && h.board.length === 5,
    )!;

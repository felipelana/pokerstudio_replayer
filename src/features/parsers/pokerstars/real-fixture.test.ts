...
const text = readFileSync(
  join(__dirname, 'fixtures', 'real-ko-tournament-dealer-export.txt'),
  'utf8',
);
...
describe('real fixture: KO tournament dealer export', () => {
  ...
  it('parses every hand with zero warnings and zero engine warnings', () => {
    ...
    for (const h of hands) {
      ...
      for (const w of r.warnings)
        if (!h.warnings.includes(w)) problems.push(`#${h.handNumber} engine: ${w}`);
    }
  ...
  it('keeps the hyphen-leading player name and sitting-out players', () => {
    ...
    expect(hands.some((h) => h.players.some((p) => p.name === 'shogi2' && p.isSittingOut))).toBe(
      true,
    );
    expect(
      hands.some((h) => h.events.some((e) => e.type === 'timeout' && e.player === 'shogi2')),
    ).toBe(true);
  });
  ...
  it('the WBreKoZo elimination hand (showdown) reveals cards and pays the right winner', () => {
    ...
    const h = hands.find((x) =>
      x.events.some((e) => e.detail?.startsWith('bounty:') && e.detail.includes('WBreKoZo')),
    )!;
    ...
    const collects = [
      ...
      ...(h.streets.river ?? []),
      ...h.showdown,
    ].filter((a) => a.type === 'collect');
    expect(collects.length).toBeGreaterThan(0);
    const winners = new Set(collects.map((c) => c.player));
    expect(
      h.summary.pots.flatMap((p) => p.winners.map((w) => w.player)).every((w) => winners.has(w)),
    ).toBe(true);
    ...
    for (const s of shown)
      expect(end.players.find((p) => p.name === s.player)?.cards).toHaveLength(2);
  ...
  it('records eliminations and bounties as informational events', () => {
    ...
    expect(bounty.some((e) => e.player === 'Saludfresh' && e.detail?.includes('WBreKoZo'))).toBe(
      true,
    );
  });

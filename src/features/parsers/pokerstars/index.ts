...
import { parseCards } from '@/domain/model/cards';
import { parseMoney } from '@/domain/model/format';
...
const RE_SEAT = /^Seat (\d+): (.+?) \(([^() ]+) in chips(?:,\s*([^)]*?)\s*bounty)?\)(?:\s+(.*))?$/;
const RE_POST =
...
const RE_FINISHED =
  /^(.+) finished the tournament in (\d+)(?:st|nd|rd|th) place(?: and received (.+?))?\.?$/;
  ...
  parse(handText: string): Omit<Hand, 'id'> {
      ...
      if (post) {
        ...
        switch (kind) {
          ...
          case 'big blind & dead': {
            ...
            if (dead > 0)
              posts.push(act(player, 'post-dead', line, { amount: dead, isAllIn: false }));
    ...
    if (summary.pots.length === 0) {
      ...
      if (sideIndexes.length > 0) {
        ...
        summary.pots.push({
          kind: 'main',
          amount: round2(sum((src) => src === 'main pot' || src === 'pot')),
          winners: [],
        });
        for (const index of [...new Set(sideIndexes)].sort((a, b) => a - b)) {
          summary.pots.push({
            ...
            amount: round2(
              sum(
                (src) =>
                  src.startsWith('side pot') &&
                  (src.includes('-') ? +src.split('-')[1] : 1) === index,
              ),
            ),
            winners: [],
          });

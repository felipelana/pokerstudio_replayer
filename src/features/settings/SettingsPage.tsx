...
import { removeTagEverywhere, usesOfTag, type TagUse } from '@/lib/db/tagUsage';
import { DEFAULT_LOOKUP_TEMPLATE, isValidLookupTemplate } from '@/domain/model/lookup';
import i18n, { LANGUAGES } from '@/i18n';
import { getRepository } from '@/lib/db/repository';
import { DEFAULT_TAGS, useAppStore, type RendererChoice } from '@/lib/state/store';
...
function Row({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div
      className="flex items-center gap-4 border-t py-2.5 text-sm"
...
export function SettingsPage() {
  ...
  const [tab, setTab] = useState<PanelId>(
    () => asked ?? (remembered(PANEL_KEY, 'general') as PanelId),
  );
  const [openGroup, setOpenGroup] = useState(() =>
    asked
      ? (GROUPS.find((g) => g.items.some((i) => i.id === asked))?.id ?? 'preferences')
      : remembered(OPEN_KEY, 'preferences'),
  );
          ...
          {tab === 'general' && (
            <section className="panel mb-4 px-4 pb-1 pt-3">
                ...
                <select
                  ...
                  onChange={(e) =>
                    update({
                      ...
                      theme: skins.find((s) => s.id === e.target.value)?.theme ?? settings.theme,
                    })
                  }
                >
                  {skins.map((s) => (
              ...
              <Row label={t('settings.chipDisplay')}>
                <select
                  className="input"
                  value={settings.chipDisplay}
                  onChange={(e) => update({ chipDisplay: e.target.value as 'chips' | 'bb' })}
                >
          ...
          {tab === 'replay' && (
            <section className="panel mb-4 px-4 pb-1 pt-3">
              ...
              <Row label={t('settings.startAtHero')}>
                <Toggle
                  checked={settings.startAtHero}
                  onChange={(v) => update({ startAtHero: v })}
                  label={t('settings.startAtHero')}
                />
              </Row>
              <Row label={t('settings.seatDistance')}>
                <div className="flex items-center gap-2">
                  ...
                  <span className="w-10 text-right text-xs tabular-nums">
                    {Math.round(settings.seatDistance * 100)}%
                  </span>
                </div>
              </Row>
              <Row label={t('settings.rotateToHero')}>
                <Toggle
                  checked={settings.rotateToHero}
                  onChange={(v) => update({ rotateToHero: v })}
                  label={t('settings.rotateToHero')}
                />
              </Row>
              ...
              <Row label={t('settings.neon')}>
                <Toggle
                  ...
                  onChange={(v) => update({ neon: v })}
                  label={t('settings.neon')}
                />
              </Row>
              ...
              <Row label={t('settings.renderer')}>
                <select
                  ...
                  onChange={(e) => update({ renderer: e.target.value as RendererChoice })}
                >
            ...
            <section className="panel mb-4 px-4 pb-3 pt-3">
                ...
                {settings.leakTags.map((tag, i) => (
                  <div key={tag.id} className="flex items-center gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        update({
                          leakTags: settings.leakTags.map((x) =>
                            x.id === tag.id ? { ...x, color: e.target.value } : x,
                          ),
                        })
                      }
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        update({
                          leakTags: settings.leakTags.map((x) =>
                            x.id === tag.id ? { ...x, label: e.target.value } : x,
                          ),
                        })
                      }
              ...
              <div className="mt-2 flex gap-2">
                ...
                <button
                  type="button"
                  className="btn"
                  onClick={() => update({ leakTags: DEFAULT_TAGS })}
                >
                  {t('common.reset')}
                </button>
              </div>
              ...
              <Row label={t('settings.lookupTemplate')}>
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onBlur={(e) => {
                      if (!isValidLookupTemplate(e.target.value))
                        update({ playerLookupUrl: DEFAULT_LOOKUP_TEMPLATE });
                    }}
                  ...
                  <button
                    type="button"
                    className="btn"
                    onClick={() => update({ playerLookupUrl: DEFAULT_LOOKUP_TEMPLATE })}
                  >
                    {t('common.reset')}
                  </button>
                </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <ul
          className="min-h-0 flex-1 overflow-auto rounded-lg border p-2 text-sm"
          style={{ borderColor: 'var(--border)' }}
        >
          {tag.uses.map((use) => (

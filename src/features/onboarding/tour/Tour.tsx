...
import { useAppStore, type TourFlow } from '@/lib/state/store';
import { useAuthStore } from '@/lib/state/authStore';
import { IconClose, IconHelp } from '@/components/ui/icons';
...
export function Tour() {
  ...
  const finish = useCallback(() => {
    ...
    if (flow === 'replayer' && !settings.tourReplayerSeen)
      updateSettings({ tourReplayerSeen: true });
  ...
  const cardTop = spot
    ? placeAbove
      ? Math.max(12, spot.top - 200)
      : below
    : Math.max(12, viewportH / 2 - 110);
  const cardLeft = spot
    ? Math.min(

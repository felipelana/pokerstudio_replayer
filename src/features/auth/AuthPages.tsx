    ...
    <div className="auth-theme flex h-full items-center justify-center overflow-auto p-6">
      <div className="w-full max-w-[420px]">
        ...
        <div className="panel p-6">
          ...
          <h1 className={subtitle ? `text-xl font-semibold` : `mb-4 text-xl font-semibold`}>
            {title}
          </h1>
          {subtitle && (
    ...
    <AuthShell title={t('auth.loginTitle')}>
      {oauthError && (
        <p
          ...
          role="alert"
          style={{ background: 'color-mix(in srgb, var(--result-lost) 18%, transparent)' }}
        >
...
function ConnectedAccounts({ identities }: { identities: string[] }) {
  ...
  const rows: { provider: ProviderName; name: 'GOOGLE' | 'FACEBOOK' | 'APPLE'; label: string }[] = [
    ...
    (row) => available[row.provider as ProviderName] || identities.includes(row.name),
  ) as never;
...
export function AccountPage() {
  ...
  const [tab, setTab] = useState<'profile' | 'security' | 'skins' | 'referral' | 'sessions'>(
    'profile',
  );
        ...
        <section className="panel p-4">
          ...
          <ul className="text-sm">
            {sessions.map((s) => (
              <li
                key={s.id}
                className="flex items-center gap-2 border-t py-1"

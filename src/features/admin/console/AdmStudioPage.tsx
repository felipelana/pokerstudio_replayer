...
import {
  ...
  type AdminUserRow,
} from '@/lib/infrastructure/http/adminApi';
import { ApiError } from '@/lib/infrastructure/http/client';
import {
  feedbackApi,
  type AdminFeedbackRow,
  type FeedbackStatus,
} from '@/lib/infrastructure/http/feedbackApi';
...
function UsersTab() {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      >
        <label className="flex flex-col text-sm">
          ...
          <input
            ...
            placeholder={t('admstudio.searchHint')}
          />
        </label>
        <label className="flex flex-col text-sm">
      ...
      {selected && (
        <UserDrawer id={selected} onClose={() => setSelected(undefined)} onChanged={reload} />
      )}
    </div>
  );
}
          ...
          <>
            <section className="panel p-3 text-sm">
              ...
              >
                ...
                <Field label={t('admstudio.colCountry')}>
                  {countryName(data.user.countryCode, i18n.language)}
                </Field>
            ...
            <section className="flex flex-wrap items-end gap-2">
              {data.user.status === 'BLOCKED' ? (
                <button
                  type="button"
                  className="btn"
                  onClick={() => run(() => adminApi.unblock(id))}
                >
                  {t('admstudio.unblock')}
                </button>
              ) : (
              ...
              <button
                type="button"
                className="btn"
                onClick={() => run(() => adminApi.revokeSessions(id))}
              >
            ...
            <section>
              ...
              <ul className="flex flex-col gap-1 text-xs">
                {data.sessions.length === 0 && (
                  <li style={{ color: 'var(--text-muted)' }}>{t('admstudio.nothing')}</li>
                )}
  ...
  return (
    <ul className="flex flex-col text-xs">
      {rows.map((row) => (
        <li
          key={row.id}
          className="flex flex-wrap items-center gap-x-3 gap-y-1 px-2 py-1.5"
        ...
        >
          <span
            ...
            style={{
              color:
                row.event.endsWith('FAIL') || row.event.includes('BLOCK')
                  ? 'var(--result-lost)'
                  : 'var(--text)',
            }}
          >
...
function EmailSettingsForm() {
  const { t } = useTranslation();
  const { data, error, busy, reload } = useLoader<AdminEmailSettings>(
  ...
  const save = async () => {
    ...
    try {
      ...
      setState({
        busy: false,
        message: err instanceof ApiError ? err.problem.title : t('auth.offline'),
        failed: true,
      });
    }
  };
    ...
    try {
      ...
      setState({
        ...
        message: result.delivered
          ? t('admstudio.testSent')
          : (result.detail ?? t('admstudio.testFailed')),
    ...
    <section className="panel p-4">
      ...
      >
        <label className="flex flex-col gap-1 text-sm">
          ...
          <select
            className="input"
            value={form.provider}
            onChange={(e) => set('provider', e.target.value as AdminEmailSettings['provider'])}
          >
      ...
      {state.message && (
        <p
          ...
          role="alert"
          style={{ color: state.failed ? 'var(--result-lost)' : 'var(--result-won)' }}
        >
          {state.message}
        </p>
      )}
    ...
    <article className="panel p-4">
      <header className="flex flex-wrap items-baseline gap-2">
        <span
          ...
          style={{ color: FEEDBACK_COLOR[row.status] }}
        >

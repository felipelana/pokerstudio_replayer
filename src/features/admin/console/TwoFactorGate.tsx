...
import { api, ApiError } from '@/lib/infrastructure/http/client';
...
const twoFactorApi = {
  ...
  verify: (code: string) =>
    api.post<{ confirmed: boolean; recoveryCodes?: string[] }>('/admin/2fa/verify', { code }),
  ...
  if (!status) {
    return (
      <p
        ...
        style={{ color: error ? 'var(--result-lost)' : 'var(--text-muted)' }}
      >
    ...
    <div className="mx-auto max-w-[520px]">
      <div className="panel p-5">
        <h2 className="text-lg font-semibold">
          {status.enrolled ? t('twofa.verifyTitle') : t('twofa.setupTitle')}
        </h2>
        <p className="mt-1 text-sm" style={{ color: 'var(--text-muted)' }}>
        ...
        {enrolment && (
          <div className="mt-4 flex flex-col gap-2">
            ...
            <code
              className="select-all break-all rounded-md px-2 py-2 font-mono text-sm"
              style={{ background: 'var(--surface-2)' }}
            >

...
import { RANKS } from '@/domain/model/cards';
import { buildReplay } from '@/domain/engine/replay';
import { computePositions } from '@/domain/model/positions';
import { pokerStarsParser } from '@/features/parsers/pokerstars';
import { getRepository } from '@/lib/db/repository';
import { anchorSeatFor, computeSeatSlots } from '@/features/replayer/renderers/layout';
import { TABLE_SHAPES } from '@/features/replayer/renderers/tableShape';
import { TableSurface } from '@/features/replayer/renderers/TableSurface';
import { formatAmount } from '@/domain/model/format';
import { BACK_PRESETS, BUILT_IN_SKINS, DECK_PRESETS, SKIN_DEFAULT_DARK } from '@/lib/skins/presets';
import {
  CHIP_DENOMINATIONS,
  type BackPattern,
  type DeckStyle,
...
import { Card } from '@/features/replayer/cards/Card';
...
function useDemo() {
  return useMemo(() => {
    ...
    const frame =
      replay.frames.find((f) => f.street === 'flop' && f.action?.type === 'call') ??
      replay.frames[replay.frames.length - 2];
...
function ColorField({
  ...
  return (
    <label className="flex items-center gap-2 py-1 text-xs">
      ...
      />
      <input
        className="input !w-[130px] !py-0.5 font-mono text-[11px]"
        value={value}
        onChange={(e) => onChange(e.target.value)}
      />
    </label>
...
function SelectField<T extends string>({
  ...
  return (
    <label className="flex items-center gap-2 py-1 text-xs">
      <span className="flex-1">{label}</span>
      <select
        className="input !w-[170px] !py-0.5"
        value={value}
...
function defaultBevel(skin: Skin): NonNullable<Skin['table']['bevel']> {
  return (
    skin.table.bevel ?? { width: 0.012, color: 'rgba(255,255,255,0.14)', opacity: 0.9, inset: 0.1 }
  );
}
...
export function AdminPage() {
  ...
  const [draft, setDraft] = useState<Skin>(
    () => skins.find((s) => s.id === settings.skinId) ?? SKIN_DEFAULT_DARK,
  );
  ...
  const patch = <K extends keyof Skin>(key: K, value: Partial<Skin[K]>) => {
    setDraft((d) => ({
      ...d,
      [key]: typeof d[key] === 'object' ? { ...(d[key] as object), ...(value as object) } : value,
    }));
    setDirty(true);
  };
  ...
  const discard = () => {
    const stored =
      skins.find((s) => s.id === draft.id) ??
      BUILT_IN_SKINS.find((s) => s.id === draft.derivedFrom);
    if (!stored) return;
    setDraft(stored);
    setDirty(false);
  ...
  const uploadBackground = async (file: File) => {
    ...
    await getRepository().saveAsset(id, file);
    patch('ui', {
      background: { ...defaultBackground(draft.ui), mode: 'image', imageAssetId: id },
    });
  };
  ...
  const slots = useMemo(
    () =>
      computeSeatSlots({
        ...
        anchorSeat: anchorSeatFor(hand, 'Hero'),
  ...
  const fmt = (v: number) =>
    formatAmount(v, { display: 'chips', bb: hand.blinds.bb, currency: 'chips' });
  const exact = (v: number) =>
    formatAmount(v, { display: 'chips', bb: hand.blinds.bb, currency: 'chips', compact: false });
  ...
  return (
    <div className="flex h-full min-h-0">
      ...
      {/* ---------- form ---------- */}
      <div
        className="flex w-[400px] shrink-0 flex-col gap-2 overflow-auto border-r p-3"
      ...
      >
        ...
        <select
          className="input"
          value={skins.some((s) => s.id === draft.id) ? draft.id : ''}
        ...
        <div className="grid grid-cols-2 gap-1.5">
          <button
            ...
            onClick={() => void save()}
            disabled={!dirty && skins.some((s) => s.id === draft.id)}
          >
            <IconSave size={14} />
            {t('common.save')}
          </button>
          <button
            ...
            onClick={() => void applyToReplayer()}
            disabled={isInUse && !dirty}
          >
            <IconCheck size={14} />
        ...
        <Section title={t('admin.deck.title')} defaultOpen>
          <div className="mb-2 grid grid-cols-2 gap-1">
            {DECK_PRESETS.map((p) => (
              <button
                key={p.id}
                type="button"
                className="btn justify-center !px-2 !py-1 text-[11px]"
                onClick={() => patch('deck', p.deck)}
              >
                {p.name}
              </button>
            ))}
          </div>
          <SelectField<DeckStyle>
            label={t('admin.deck.style')}
            value={draft.deck.style}
            options={[
              { value: 'filled', label: t('admin.deck.filled') },
              { value: 'outlined', label: t('admin.deck.outlined') },
            ]}
            onChange={(v) => patch('deck', { style: v })}
          />
          <SelectField
            label={t('admin.deck.colorMode')}
            value={String(draft.deck.colorMode)}
            options={[
              { value: '2', label: t('admin.deck.twoColor') },
              { value: '4', label: t('admin.deck.fourColor') },
            ]}
            onChange={(v) => {
          ...
          />
          <ColorField
            label={t('admin.deck.spades')}
            value={draft.deck.suitColors.s}
            onChange={(v) => patch('deck', { suitColors: { ...draft.deck.suitColors, s: v } })}
          />
          <ColorField
            label={t('admin.deck.hearts')}
            value={draft.deck.suitColors.h}
            onChange={(v) => patch('deck', { suitColors: { ...draft.deck.suitColors, h: v } })}
          />
          <ColorField
            label={t('admin.deck.diamonds')}
            value={draft.deck.suitColors.d}
            onChange={(v) => patch('deck', { suitColors: { ...draft.deck.suitColors, d: v } })}
          />
          <ColorField
            label={t('admin.deck.clubs')}
            value={draft.deck.suitColors.c}
            onChange={(v) => patch('deck', { suitColors: { ...draft.deck.suitColors, c: v } })}
          />
          <ColorField
            label={t('admin.deck.cardBg')}
            value={draft.deck.cardBg}
            onChange={(v) => patch('deck', { cardBg: v })}
          />
          <ColorField
            label={t('admin.deck.inkOnFilled')}
            value={draft.deck.inkOnFilled}
            onChange={(v) => patch('deck', { inkOnFilled: v })}
          />
          <ColorField
            label={t('admin.deck.backColor')}
            value={draft.deck.backColor}
            onChange={(v) => patch('deck', { backColor: v })}
          />
          <ColorField
            label={t('admin.deck.backInk')}
            value={draft.deck.backInk}
            onChange={(v) => patch('deck', { backInk: v })}
          />

          <SelectField<BackPattern>
            label={t('admin.deck.backPattern')}
            value={draft.deck.backPattern}
            options={(['diamonds', 'grid', 'dots', 'plain'] as BackPattern[]).map((p) => ({
              value: p,
              label: t(`admin.deck.pattern.${p}`),
            }))}
            onChange={(v) => patch('deck', { backPattern: v })}
          />
          <SelectField
          ...
          />
          <SelectField<RankFont>
            label={t('admin.deck.rankFont')}
            value={draft.deck.rankFont}
            options={[
              { value: 'Inter', label: 'Inter' },
              { value: 'Roboto Mono', label: 'Roboto Mono' },
              { value: 'serif', label: 'Serif' },
            ]}
            onChange={(v) => patch('deck', { rankFont: v })}
          />
          <RangeField
            label={t('admin.deck.cornerRadius')}
            value={draft.deck.cornerRadius}
            min={0}
            max={0.3}
            step={0.01}
            onChange={(v) => patch('deck', { cornerRadius: v })}
          />
          <RangeField
            label={t('admin.deck.boardGap')}
            value={draft.deck.boardGap ?? 0.36}
            min={0}
            max={0.6}
            step={0.02}
            onChange={(v) => patch('deck', { boardGap: v })}
          />
              ...
              return (
                ...
                >
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      patch('deck', {
                        rankColors: { ...draft.deck.rankColors, [rank]: e.target.value },
                      })
                    }
        ...
        <Section title={t('admin.felt.title')}>
          <ColorField
            label={t('admin.felt.color')}
            value={draft.felt.color}
            onChange={(v) => patch('felt', { color: v })}
          />
          <RangeField
            label={t('admin.felt.textureIntensity')}
            value={draft.felt.textureIntensity}
            min={0}
            max={1}
            step={0.05}
            onChange={(v) => patch('felt', { textureIntensity: v })}
          />
          <ColorField
            label={t('admin.felt.vignetteColor')}
            value={draft.felt.vignetteColor}
            onChange={(v) => patch('felt', { vignetteColor: v })}
          />
          <RangeField
            label={t('admin.felt.vignetteStrength')}
            value={draft.felt.vignetteStrength}
            min={0}
            max={1}
            step={0.05}
            onChange={(v) => patch('felt', { vignetteStrength: v })}
          />
          <label className="flex items-center gap-2 py-1 text-xs">
            <span className="flex-1">{t('admin.felt.logoText')}</span>
            <input
              className="input !w-[170px] !py-0.5"
              value={draft.felt.logoText ?? ''}
              onChange={(e) => patch('felt', { logoText: e.target.value || undefined })}
            />
          </label>
          <RangeField
            label={t('admin.felt.logoOpacity')}
            value={draft.felt.logoOpacity}
            min={0}
            max={1}
            step={0.05}
            onChange={(v) => patch('felt', { logoOpacity: v })}
          />
          <SelectField
          ...
          <div className="flex items-center gap-2 py-1 text-xs">
            ...
            {draft.felt.logoAssetId && (
              <button
                type="button"
                className="btn !py-0.5 text-[11px]"
                onClick={() => patch('felt', { logoAssetId: undefined })}
              >
        ...
        <Section title={t('admin.table.title')}>
          <ColorField
            label={t('admin.table.railColor')}
            value={draft.table.railColor}
            onChange={(v) => patch('table', { railColor: v })}
          />
          <ColorField
            label={t('admin.table.railHighlight')}
            value={draft.table.railHighlight}
            onChange={(v) => patch('table', { railHighlight: v })}
          />
          <RangeField
            label={t('admin.table.railWidth')}
            value={draft.table.railWidth}
            min={0.02}
            max={0.12}
            step={0.005}
            onChange={(v) => patch('table', { railWidth: v })}
          />
          <RangeField
            label={t('admin.table.railShine')}
            value={draft.table.railShine}
            min={0}
            max={1}
            step={0.05}
            onChange={(v) => patch('table', { railShine: v })}
          />
          <RangeField
            label={t('admin.table.scale')}
            value={draft.table.scale ?? 1}
            min={0.8}
            max={1.35}
            step={0.01}
            onChange={(v) => patch('table', { scale: v })}
          />
          <RangeField
            label={t('admin.table.aspect')}
            value={draft.table.aspect}
            min={0.4}
            max={0.75}
            step={0.01}
            onChange={(v) => patch('table', { aspect: v })}
          />
          <ColorField
            label={t('admin.table.neonColor')}
            value={draft.table.neonColor ?? draft.plates.activeBorder}
            onChange={(v) => patch('table', { neonColor: v })}
          />
          <SelectField
          ...
          <RangeField
            ...
            step={0.002}
            onChange={(v) =>
              patch('table', { bevel: v === 0 ? undefined : { ...defaultBevel(draft), width: v } })
            }
          />
          <RangeField
          ...
          />
          <RangeField
            label={t('admin.table.neonIntensity')}
            value={draft.table.neonIntensity ?? 0}
            min={0}
            max={1}
            step={0.05}
            onChange={(v) => patch('table', { neonIntensity: v })}
          />
        </Section>

        <Section title={t('admin.ui.title')}>
          {(
            ['bg', 'bgEnd', 'surface', 'surface2', 'text', 'textMuted', 'accent', 'border'] as const
          ).map((k) => (
            <ColorField
              key={k}
              label={t(`admin.ui.${k}`)}
              value={draft.ui[k]}
              onChange={(v) => patch('ui', { [k]: v })}
            />
          ))}
        </Section>
        ...
        <Section title={t('admin.chips.title')}>
          {CHIP_DENOMINATIONS.map((d) => (
            <ColorField
              key={d}
              label={t('admin.chips.denomination', { value: d >= 1000 ? `${d / 1000}K` : d })}
              value={draft.chips.colors[String(d)] ?? '#888888'}
              onChange={(v) =>
                patch('chips', { colors: { ...draft.chips.colors, [String(d)]: v } })
              }
            />
          ))}
          <ColorField
            label={t('admin.chips.edge')}
            value={draft.chips.edge}
            onChange={(v) => patch('chips', { edge: v })}
          />
          <ColorField
            label={t('admin.chips.dealerButton')}
            value={draft.chips.dealerButton}
            onChange={(v) => patch('chips', { dealerButton: v })}
          />
          <ColorField
            label={t('admin.chips.dealerButtonInk')}
            value={draft.chips.dealerButtonInk}
            onChange={(v) => patch('chips', { dealerButtonInk: v })}
          />
        </Section>
        ...
        <Section title={t('admin.background.title')}>
          <SelectField
            ...
            onChange={(v) =>
              patch('ui', {
                background: {
                  ...defaultBackground(draft.ui),
                  mode: v as 'color' | 'gradient' | 'image',
                },
              })
            }
          />
          <ColorField
            ...
            onChange={(v) =>
              patch('ui', { background: { ...defaultBackground(draft.ui), color: v } })
            }
          />
                ...
                onChange={(v) => {
                  ...
                  patch('ui', {
                    background: {
                      ...
                      gradient: {
                        ...b.gradient!,
                        stops: [b.gradient!.stops[0], { color: v, at: 1 }],
                      },
                    },
                  });
                }}
          ...
          {(draft.ui.background?.mode ?? 'color') === 'image' && (
            <>
              <div className="flex items-center gap-2 py-1 text-xs">
                ...
                {draft.ui.background?.imageAssetId && (
                  <button
                    ...
                    onClick={() =>
                      patch('ui', {
                        background: { ...defaultBackground(draft.ui), imageAssetId: undefined },
                      })
                    }
                  >
                    {t('admin.felt.removeLogo')}
              ...
              <SelectField
                ...
                options={(['cover', 'contain', 'repeat', 'center'] as const).map((v) => ({
                  value: v,
                  label: t(`admin.background.fit_${v}`),
                }))}
                onChange={(v) =>
                  patch('ui', {
                    background: {
                      ...defaultBackground(draft.ui),
                      imageFit: v as 'cover' | 'contain' | 'repeat' | 'center',
                    },
                  })
                }
              />
              <RangeField
                ...
                step={0.05}
                onChange={(v) =>
                  patch('ui', { background: { ...defaultBackground(draft.ui), imageOpacity: v } })
                }
              />
              <RangeField
                ...
                step={1}
                onChange={(v) =>
                  patch('ui', { background: { ...defaultBackground(draft.ui), blur: v } })
                }
              />
            </>
          )}
          <button
            type="button"
            className="btn !py-0.5 text-[11px]"
            onClick={() => patch('ui', { background: undefined })}
          >
            {t('common.reset')}
          </button>
        </Section>
        ...
        <Section title={t('admin.logo.title')}>
          <div className="flex items-center gap-2 py-1 text-xs">
            ...
            <button
              type="button"
              className="btn !py-0.5 text-[11px]"
              onClick={() => cornerLogoInput.current?.click()}
            >
              {t('common.import')}
            </button>
            {draft.ui.logoAssetId && (
              <button
                type="button"
                className="btn !py-0.5 text-[11px]"
                onClick={() => patch('ui', { logoAssetId: undefined })}
              >
                {t('admin.felt.removeLogo')}
              </button>
            )}
          ...
          <SelectField
            ...
            options={(
              ['none', 'top-left', 'top-right', 'bottom-left', 'bottom-right'] as const
            ).map((c) => ({
              value: c,
          ...
          />
          <RangeField
            label={t('admin.logo.size')}
            value={draft.ui.logoSize ?? 44}
            min={20}
            max={140}
            step={2}
            onChange={(v) => patch('ui', { logoSize: v })}
          />
          <RangeField
            label={t('admin.logo.opacity')}
            value={draft.ui.logoOpacity ?? 0.85}
            min={0.1}
            max={1}
            step={0.05}
            onChange={(v) => patch('ui', { logoOpacity: v })}
          />
        </Section>
        ...
        <Section title={t('admin.plates.title')}>
          {(
            ...
            <ColorField
              key={k}
              label={t(`admin.plates.${k}`)}
              value={draft.plates[k]}
              onChange={(v) => patch('plates', { [k]: v })}
            />
          ))}
          <ColorField
      ...
      {/* ---------- preview ---------- */}
      <div
        className="flex min-w-0 flex-1 flex-col gap-3 overflow-auto p-4"
        style={{
          background: `linear-gradient(180deg, ${draft.ui.bg}, ${draft.ui.bgEnd})`,
          color: draft.ui.text,
        }}
      >
            ...
            <div className="flex gap-1">
              {BACK_PRESETS.map((b) => (
                <button
                  ...
                  style={{
                    outline:
                      draft.deck.backPattern === b.id ? '2px solid var(--accent)' : undefined,
                    outlineOffset: 2,
                    borderRadius: 4,
                  }}
                >
        ...
        <div
          ...
          style={{
            ...
            transform: `scale(${draft.table.scale ?? 1})`,

    ...
    (summaries: ImportSummary[]) => {
      ...
      for (const s of summaries) {
        if (s.result.hands.length === 0) {
          if (s.result.site === 'unknown')
            msgs.push({ kind: 'error', text: `${s.session.name}: ${t('library.unknownFormat')}` });
            ...
            msgs.push({ kind: 'warn', text: t('library.nothingParsed', { name: s.session.name }) });
          continue;
        }
  ...
  const handleFiles = useCallback(
    async (files: File[]) => {
      ...
      try {
        ...
        const [first, ...rest] = files.filter(
          (f) => /\.txt$/i.test(f.name) || f.type.startsWith('text/'),
        );
        if (!first) return;
        ...
        if (rest.length) {
          setMessages((current) => [
            ...
            {
              kind: 'warn',
              text: t('library.onlyFirstFile', { count: rest.length, name: first.name }),
            },
          ]);
        }
      } catch (e) {
      ...
      <div className="flex flex-1 flex-col gap-3">
        ...
        <label className="flex items-center gap-2 text-xs" style={{ color: 'var(--text-muted)' }}>
          ...
          <select
            ...
            value={override}
            onChange={(e) => setOverride(e.target.value as Site | '')}
          >

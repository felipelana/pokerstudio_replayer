...
import { readableInk, textHalo } from '@/components/ui/contrast';
...
import {
  ...
  cardPrimitives,
  patternShapes,
} from '@/features/replayer/cards/primitives';
                  ...
                  {patternShapes(p.pattern, p.inset).map((sh, j) =>
                    sh.kind === 'circle' ? (
                      ...
                      <path
                        key={j}
                        stroke="none"
                        d={`M${sh.x} ${sh.y - sh.h / 2} L${sh.x + sh.w / 2} ${sh.y} L${sh.x} ${sh.y + sh.h / 2} L${sh.x - sh.w / 2} ${sh.y} Z`}
                      />
                    ) : (
      ...
      >
        <defs>
          <radialGradient id={feltId} cx="50%" cy="45%" r="60%">
            ...
            <stop
              offset="100%"
              stopColor={skin.felt.vignetteColor}
              stopOpacity={skin.felt.vignetteStrength}
            />
          </radialGradient>
          <linearGradient id={`${feltId}-rail`} x1="0" y1="0" x2="0" y2="1">
            <stop
              offset="0%"
              stopColor={skin.table.railHighlight}
              stopOpacity={0.5 + skin.table.railShine * 0.5}
            />
            <stop offset="55%" stopColor={skin.table.railColor} />
            <stop
              offset="100%"
              stopColor={skin.table.railHighlight}
              stopOpacity={skin.table.railShine * 0.5}
            />
          </linearGradient>
        ...
        {neonStrength > 0 && (
          <g clipPath={`url(#${feltId}-clip)`}>
            <path
              d={shapeSvgPath(shape, CX, CY, RX * 0.9, RY * 0.9)}
              fill="none"
              stroke={neonColor}
              strokeWidth={16}
        ...
        {bevel ? (
          <g>
            ...
            <path
              d={shapeSvgPath(
                ...
                CX,
                CY,
                RX - RY * bevel.inset - 1.5,
                RY - RY * bevel.inset - 1.5,
              )}
              fill="none"
          ...
          neonStrength === 0 && (
            <path
              d={shapeSvgPath(shape, CX, CY, RX - 22, RY - 18)}
              fill="none"
        ...
        {slots.map((slot) => {
          ...
          const button = avoidZones(
            { x: slot.buttonX, y: slot.buttonY, hw: 13 / RX, hh: 13 / RY },
            [boardZone],
          );
          return (
            <g key={slot.seat}>
              ...
              {hand.buttonSeat === slot.seat && (
                <g transform={`translate(${CX + button.x * RX} ${CY + button.y * RY})`}>
                  ...
                  <text
                    ...
                    fontSize={13}
                    fontWeight={800}
                    fill={skin.chips.dealerButtonInk}
                  >
                    {t('table.dealer')}
                  </text>
                </g>

...
import { contrastRatio, readableInk } from '@/components/ui/contrast';
    ...
    for (let a = 0; a < 24; a++) {
      ...
      for (const radius of [0.8, 0.95, 1.1, 1.4]) {
        for (const hw of [0.04, 0.12, 0.2]) {
          ...
          expect(cornerRadius(box, at), `angle ${a} r ${radius} hw ${hw}`).toBeLessThanOrEqual(
            1.0001,
          );
        }
      }
...
describe('computeSeatSlots: where the hero sits', () => {
  ...
  it('wraps a slot past the last chair, and ignores the choice with no hero', () => {
    expect(
      computeSeatSlots({ maxSeats: 6, anchorSeat: 1, heroSlot: 8 }).find((s) => s.seat === 1)
        ?.index,
    ).toBe(2);

...
import { haloShadow, readableInk } from '@/components/ui/contrast';
import { SeatPlate, seatLabels, type SeatLabels } from '../seats/SeatPlate';
import { SvgTableRenderer } from '../svg/SvgTableRenderer';
import { cardCanvas } from '@/features/replayer/cards/cardTexture';
import type { DeckSkin, FeltSkin, Skin, TableSkin } from '@/lib/skins/types';
import { CARD_H, CARD_W } from '@/features/replayer/cards/primitives';
...
function woodTexture(table: TableSkin): THREE.CanvasTexture {
  ...
  if (!tex) {
    ...
    for (let i = 0; i < 260; i++) {
      ...
      ctx.strokeStyle =
        rand() > 0.45 ? hexToRgba(table.railHighlight, alpha) : `rgba(0,0,0,${alpha * 0.8})`;
      ctx.lineWidth = 0.6 + rand() * 2.2;
      ctx.beginPath();
      ctx.moveTo(0, y);
      for (let x = 0; x <= w; x += 32)
        ctx.lineTo(x, y + Math.sin((x / w) * Math.PI * (1 + rand()) + i) * amp);
      ctx.stroke();
    }
...
function chipTopTexture(color: string, denom: number): THREE.CanvasTexture {
  ...
  if (!tex) {
    ...
    ctx.setLineDash([]);
    const label =
      denom >= 1e6 ? `${denom / 1e6}M` : denom >= 1000 ? `${denom / 1000}K` : String(denom);
    ...
    <group>
      ...
      {skin.table.bevel && bevelGeo && (
        <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.005, 0]} geometry={bevelGeo}>
          <meshBasicMaterial
            color={skin.table.bevel.color}
            transparent
            opacity={skin.table.bevel.opacity}
            depthWrite={false}
          />
        </mesh>
      )}
...
function CardMesh({
  ...
  return (
    <group
      ...
      rotation={[-Math.PI / 2 + CARD_TILT, 0, rotationY]}
      scale={scale}
    >
        ...
        <mesh position={[0, 0.03 + chips.length * 0.06 + 0.001, 0]} rotation={[-Math.PI / 2, 0, 0]}>
          {denominations ? (
            <>
              ...
              <meshBasicMaterial
                map={chipTopTexture(skin.chips.colors[String(top)] ?? '#888', top)}
              />
            </>
          ) : (
            <>
...
function Scene(props: TableRendererProps & { labels: SceneLabels; feltLogo?: HTMLImageElement }) {
  ...
  const winners = new Set(
    frame.kind === 'end' ? frame.players.filter((p) => p.collected > 0).map((p) => p.name) : [],
  );
    ...
    <>
      ...
      {(hideBoard ? [] : frame.board).map((c, i) => (
        <Appear key={c} enabled={animations} delay={i * 40}>
          <CardMesh
            card={c}
            deck={skin.deck}
            position={[boardX0 + i * (boardCardW + boardGap), 0.05, 0]}
            scale={zoomCards}
            art={deckArt?.[c[0]]}
          />
        </Appear>
      ...
      {frame.pot > 0 && (
        <Appear enabled={animations}>
          <ChipStack3D
            chips={chipBreakdown(frame.pot, isCash, 10)}
            skin={skin}
            position={[-2.9, 0, POT_Z]}
            zoom={zoomChips}
      ...
      {slots.map((slot) => {
        ...
        // board / pot bands (R4).
        const stackTop = -(
          (p?.streetBet ? chipBreakdown(p.streetBet, isCash).length : 0) * 0.06 +
          0.12
        );
        ...
        const button = avoidZones(
          { x: slot.buttonX, y: slot.buttonY, hw: 0.28 / RX, hh: 0.28 / rz },
          [boardZone],
        );
        ...
        return (
          <group key={slot.seat}>
            {p && p.streetBet > 0 && (
              <Appear enabled={animations}>
                <ChipStack3D
                  chips={chipBreakdown(p.streetBet, isCash)}
                  skin={skin}
                  position={[bx, 0, bz]}
                  zoom={zoomChips}
                  denominations={chipDenominations}
            ...
            {hand.buttonSeat === slot.seat && (
              <DealerButton
                skin={skin}
                position={[button.x * RX, 0.03, button.y * rz]}
                label={labels.dealer}
              />
            )}
            <Html
            ...
            >
              {p ? (
                <SeatPlate
                  ...
                  onSitHere={
                    interactive && onSitHere && p.name !== heroName
                      ? () => onSitHere(slot.index)
                      : undefined
                  }
...
export function ThreeTableRenderer(props: TableRendererProps) {
  ...
  const feltLogo = useAssetImage(
    props.skin.felt.logoAssetId,
    props.skin.felt.logoBlend === 'screen',
  );
  ...
  const camera = useMemo(
    () => ({
      position: [0, 9.9, rz + 7.1] as [number, number, number],
      fov: 43,
      near: 0.1,
      far: 100,
    }),

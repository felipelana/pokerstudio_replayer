...
import type { HandReplay } from '@/domain/engine/replay';
import { useAppStore, type JumpTarget } from '@/lib/state/store';
import {
  IconFirst,
  IconLast,
  IconNext,
...
export function Footer({ replay, rows, visible, currentIndex, fmt, onSelectHand }: Props) {
  ...
  return (
    <footer
      className="flex h-[182px] shrink-0 flex-col gap-1.5 border-t p-2"
        ...
        <div className="flex w-[420px] shrink-0 flex-col gap-2">
          <div className="flex gap-1" data-tour="streets">
            {JUMPS.map((j) => (
              <button
                ...
                disabled={jumpTargets[j.target] === undefined}
                onClick={() => jumpTo(j.target)}
                title={`${t(j.label)} (${j.key})`}
              >
                {t(j.label)}
              </button>
            ))}
          ...
          <div className="flex items-center gap-1.5" data-tour="transport">
            ...
            <button
              type="button"
              className="btn-icon"
              onClick={() => setFrame(replay.frames.length - 1)}
              title={t('footer.last')}
              aria-label={t('footer.last')}
            >
              <IconLast size={15} />
            </button>

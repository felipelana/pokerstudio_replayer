...
import { useActiveSkin, useAppStore } from '@/lib/state/store';
        ...
        >
          ...
          <label className="flex flex-col gap-1">
            ...
            <select
              ...
              onChange={(e) =>
                update({
                  holeLayoutOverride: e.target.value as 'skin' | 'spread' | 'overlap' | 'fan',
                })
              }
            >
              <option value="skin">{t('quick.layoutSkin')}</option>
          ...
          <label className="checkbox">
            <input
              type="checkbox"
              checked={settings.hideHeroCards}
              onChange={(e) => update({ hideHeroCards: e.target.checked })}
            />
            {t('quick.hideHero')}
          </label>
          ...
          <label className="checkbox">
            <input
              type="checkbox"
              checked={settings.chipDenominations}
              onChange={(e) => update({ chipDenominations: e.target.checked })}
            />
          ...
          {(
            ...
            <label key={key} className="flex items-center gap-1.5">
              <span
                className="w-[52px] shrink-0 text-[10.5px] leading-tight"
                style={{ color: 'var(--text-muted)' }}
              >
                {t(label)}
              </span>
          ...
          <label className="flex items-center gap-2">
            ...
            <input
              ...
              step={0.02}
              value={
                settings.boardGapOverride === 'skin'
                  ? (skin.deck.boardGap ?? 0.36)
                  : settings.boardGapOverride
              }
          ...
          <label
            className="checkbox"
            title={knownVillainCards ? undefined : t('quick.revealDisabled')}
          >
            <input
              type="checkbox"
            ...
            />
            <span style={{ opacity: knownVillainCards ? 1 : 0.5 }}>
              {t('quick.revealVillains')}
            </span>
          </label>
        </div>
      )}

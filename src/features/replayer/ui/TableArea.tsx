...
import { DECK_PRESETS } from '@/lib/skins/presets';
import { useDeckArt } from '@/components/hooks/useDeckArt';
import { useTranslation } from 'react-i18next';
import type { Frame, HandReplay } from '@/domain/engine/replay';
import { potOddsFor } from '@/domain/engine/replay';
import { siteName } from '@/domain/model/sites';
import { buildLookupUrl } from '@/domain/model/lookup';
import { IconCompress, IconExpand } from '@/components/ui/icons';
...
import type { TableRendererProps } from '@/features/replayer/renderers/TableRenderer';
import { TableSurface } from '@/features/replayer/renderers/TableSurface';
import { useAppStore, useActiveSkin } from '@/lib/state/store';
import { useAmountFormatter, useDateFormatter } from '@/components/hooks/useFormat';
...
export function TableArea({ replay, frame, heroName, onSeatClick }: Props) {
  ...
  const skin = useMemo(() => {
    ...
    return preset
      ? { ...baseSkin, deck: { ...preset.deck, boardGap: baseSkin.deck.boardGap } }
      : baseSkin;
  ...
  return (
    <div className="relative flex h-full min-w-0 flex-1 flex-col" data-tour="table">
      ...
      >
        ...
        {potOdds && (
          <div
            className="flex items-center gap-3 rounded-md px-2 py-1 font-medium tabular-nums"
            style={{ background: 'var(--surface-2)', color: 'var(--text)' }}
          >
      ...
      <div className="relative min-h-0 flex-1 px-2 pb-1">
        <div
          ...
          style={{
            transform: `scale(${settings.zoomTable * (skin.table.scale ?? 1)})`,

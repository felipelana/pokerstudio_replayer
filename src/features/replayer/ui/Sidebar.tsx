...
import type { HandMeta } from '@/domain/engine/replay';
import type { PositionLabel } from '@/domain/model/positions';
...
export function Sidebar({
  ...
  const showResult = (row: HandRow) =>
    !settings.hideResults && settings.colorHintResults && !!row.meta.result;
  ...
  const netColor = (row: HandRow) => {
    ...
    if (settings.hideResults || row.meta.net === undefined || Math.abs(row.meta.net) < 0.005)
  ...
  const activeChips = [
    ...
    ...(filterPlayedOnly
      ? [{ key: 'played', label: t('sidebar.playedOnly'), clear: () => setFilterPlayedOnly(false) }]
      : []),
    ...(filterResult !== 'all'
      ? [
          {
            key: 'result',
            label: t(`sidebar.result${filterResult === 'won' ? 'Won' : 'Lost'}`),
    ...
    ...(sortMode !== 'default'
      ? [
          {
            key: 'sort',
            label: t(`sidebar.sort${sortMode === 'potDesc' ? 'Pot' : 'Reverse'}`),
            clear: () => setSortMode('default'),
          },
        ]
      : []),
  ];
  ...
  if (settings.sidebarCollapsed) {
    return (
      <div
        className="flex h-full shrink-0 flex-col items-center gap-2 border-r p-1.5"
    ...
    >
      ...
      <Section title={t('sidebar.moreFilters')}>
        ...
        <label className="checkbox mt-1 text-[11px]">
          <input
            type="checkbox"
            checked={filterPlayedOnly}
            onChange={(e) => setFilterPlayedOnly(e.target.checked)}
          />
      ...
      <Section title={t('sidebar.focusSelect')} defaultOpen={!sessionHasHero && !focusPlayer}>
        ...
        >
          <option value="">
            {sessionHasHero ? t('sidebar.fileHero') : t('sidebar.choosePlayer')}
          </option>
          {players.map((p) => (
        ...
        <button
          ...
          onClick={() =>
            filtered.length && onSelect(filtered[Math.floor(Math.random() * filtered.length)].i)
          }
        >
          <IconDice size={15} />
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          title={t('sidebar.copyRaw')}
          aria-label={t('sidebar.copyRaw')}
        >
      ...
      <div
        ref={listRef}
        className="-mx-0.5 min-h-0 flex-1 overflow-y-auto overflow-x-hidden"
        role="listbox"
        aria-label={t('sidebar.hands')}
      >
              ...
              >
                ...
                <span className="flex w-[48px] shrink-0 items-center gap-[2px]">
                  {r.meta.heroCards?.length ? (
                    r.meta.heroCards.map((c) => (
                      <Card key={c} card={c} deck={skin.deck} width={22} />
                    ))
                  ) : r.meta.heroName ? (
                    <>
                ...
                <span
                  ...
                  style={{ color: r.position ? 'var(--text)' : 'var(--text-muted)' }}
                >
                  {r.position ?? t('positions.none')}
                </span>
                <span
                  className="min-w-0 flex-1 text-right font-medium tabular-nums"
                  style={{ color: netColor(r) }}
                >

...
import type { Frame } from '@/domain/engine/replay';
import type { Action, Hand } from '@/domain/model/types';
...
export function describeFrame(
  ...
  switch (frame.kind) {
    ...
    case 'deal-street':
      if (frame.street === 'flop')
        return t('log.dealFlop', { cards: frame.board.slice(0, 3).join(' ') });
...
export function describeAction(a: Action, t: TFunction, fmt: (v: number) => string): string {
  ...
  switch (a.type) {
    ...
    case 'disconnect':
      return a.detail === 'connected'
        ? t('log.connected', { player })
        : t('log.disconnected', { player });
    case 'info': {
      const d = a.detail ?? '';
      if (d.startsWith('finished:')) {
        const [, place, prize] = d.split(':');
        return prize
          ? t('log.finishedPrize', { player, place, prize })
          : t('log.finished', { player, place });
      }

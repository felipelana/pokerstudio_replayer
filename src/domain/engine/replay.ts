import type { Action, Hand, HandResult, Street } from '@/domain/model/types';
...
export function buildReplay(hand: Hand, heroOverride?: string): HandReplay {
  ...
  for (const a of hand.streets.preflop)
    steps.push({ kind: 'action', action: a, street: 'preflop' });
  ...
  if (Math.abs(diff) > 0.011) {
    warnings.push(
      `Chip conservation off by ${diff.toFixed(2)} (start ${startSum}, end ${endSum}, rake ${hand.summary.rake})`,
    );
  }

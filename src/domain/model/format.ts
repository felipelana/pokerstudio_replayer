...
export function compactNumber(value: number, isCash: boolean, locale = 'en'): string {
  ...
  if (isCash) {
    if (abs >= 10_000)
      return (
        sign + new Intl.NumberFormat(locale, { maximumFractionDigits: 1 }).format(abs / 1000) + 'K'
      );
    return (
      sign +
  ...
  if (abs >= 1_000_000)
    return (
      sign +
      new Intl.NumberFormat(locale, { maximumFractionDigits: 1 }).format(abs / 1_000_000) +
      'M'
    );
  if (abs >= 10_000)
    return (
...
export function formatAmount(value: number, opts: FormatOpts): string {
  ...
  const body =
    opts.compact === false
      ? exactNumber(value, isCash, locale)
      : compactNumber(value, isCash, locale);
  return sym + body;
}

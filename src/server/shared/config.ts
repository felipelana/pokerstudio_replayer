...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): Config {
  ...
  if (parsed.GOOGLE_CLIENT_ID && !parsed.GOOGLE_CLIENT_ID.endsWith(GOOGLE_CLIENT_SUFFIX)) {
    ...
    throw new Error(
      `GOOGLE_CLIENT_ID does not look like a Google client id (it should end in ${GOOGLE_CLIENT_SUFFIX})`,
    );
  }
  return {
    ...parsed,
    ...
    facebookEnabled: !!(
      parsed.FACEBOOK_APP_ID &&
      parsed.FACEBOOK_APP_SECRET &&
      parsed.FACEBOOK_REDIRECT_URI
    ),
    appleEnabled: !!(

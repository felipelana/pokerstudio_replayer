...
export async function authRoutes(app: FastifyInstance, container: AppContainer) {
  const cookieOpts = {
    domain: container.config.COOKIE_DOMAIN,
    ...
    async (request, reply) => {
      ...
      return reply
        ...
        .send({
          ...
          emailVerificationRequired: container.requireVerification,
        });
    },
  );
  ...
  app.post(
    ...
    async (request, reply) => {
      const body = z
        .object({ email: z.string().min(3), captchaToken: z.string().optional() })
        .parse(request.body);
      const result = await requestPasswordReset(resetDeps(container), {
        ...body,
        ip: clientIp(request),
      });
      if (!result.ok) return problem(reply, result.error);
  ...
  app.patch('/auth/me', async (request, reply) => {
    ...
    const result = await updateProfile(accountDeps(container), {
      user,
      patch,
      ip: clientIp(request),
    });
  ...
  app.post('/auth/change-password', async (request, reply) => {
    ...
    const result = user.passwordHash
      ...
      : await setFirstPassword(accountDeps(container), {
          user,
          newPassword: body.newPassword,
          ip: clientIp(request),
        });
  ...
  app.delete('/auth/me', async (request, reply) => {
    ...
    const result = await deleteAccount(accountDeps(container), {
      user,
      password: body.password,
      ip: clientIp(request),
    });

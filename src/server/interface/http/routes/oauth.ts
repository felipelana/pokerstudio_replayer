...
const PATHS: Record<OAuthProviderName, string> = {
  GOOGLE: 'google',
  FACEBOOK: 'facebook',
  APPLE: 'apple',
};
...
export async function oauthRoutes(app: FastifyInstance, container: AppContainer) {
  ...
  const begin =
    (provider: OAuthProvider) => async (request: FastifyRequest, reply: FastifyReply) => {
      const query = z
  ...
  const finish =
    (provider: OAuthProvider) => async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const fallback = (code: string) =>
        reply.redirect(`${container.config.APP_URL}/login?error=${code}`);
      ...
      if (saved.state !== query.state || saved.provider !== provider.name)
        return fallback('oauth_state');
  ...
  app.delete('/auth/identities/:provider', async (request, reply) => {
    ...
    if (!user) return;
    const { provider } = z
      .object({ provider: z.enum(['GOOGLE', 'FACEBOOK', 'APPLE']) })

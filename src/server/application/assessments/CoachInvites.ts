...
export async function shareSettings(
  prisma: PrismaClient,
): Promise<{ defaultHours: number; maxHours: number }> {
...
export async function saveShareSettings(
  ...
  if (patch.defaultHours < 1 || patch.maxHours < 1)
    return fail(Errors.validation('Os prazos são de pelo menos uma hora.'));
  if (patch.defaultHours > patch.maxHours)
    return fail(Errors.validation('O prazo padrão não pode passar do prazo máximo.'));
...
> {
  ...
  if (input.handIndex !== undefined) {
    if (
      !Number.isInteger(input.handIndex) ||
      input.handIndex < 0 ||
      input.handIndex >= session.handCount
    ) {
...
export async function openInvite(
  ...
  if (!(await deps.hasher.verify(invite.passwordHash, password)))

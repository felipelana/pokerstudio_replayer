...
export async function saveHandAssessment(
  ...
  const hand = await deps.prisma.handRecord.findUnique({
    where: {
      reviewSessionId_index: {
        reviewSessionId: guard.assessment.reviewSessionId,
        index: handIndex,
      },
    },
    select: { id: true },
  ...
  const data = {
    ...
    ...(patch.streetComments !== undefined
      ? { streetComments: patch.streetComments ?? undefined }
      : {}),
 ...
 */
export async function sessionAssessments(
  deps: AssessmentDeps,
  userId: string,
  reviewSessionId: string,
) {
  ...
  return {
    ...
    items: assessments.map((a) => {
      ...
      const rows = hands.map((h) => ({
        heroVpip: h.heroVpip,
        assessment: byHand.get(h.id) ?? null,
      }));
      return {
        id: a.id,
        role: a.role,

...
export async function saveEmailSettings(
  ...
  if (settings.provider !== 'NONE' && !settings.fromAddress) {
    return fail(
      appError('from_address_required', 'Set the sender address before enabling a provider.', 422),
    );
  }
  ...
  await deps.prisma.emailSettings.upsert({
    where: { id: 1 },
    create: { id: 1, ...data },
    update: data,
  });
...
export async function sendTestEmail(
  ...
  if (row.provider === 'SMTP') {
    ...
    return fail(
      appError('smtp_not_supported', 'SMTP delivery is not implemented yet. Use Resend.', 501),
    );
  }
  ...
  try {
    ...
    await deps.prisma.emailSettings.update({
      where: { id: 1 },
      data: { lastTestAt: now, lastError: detail },
    });

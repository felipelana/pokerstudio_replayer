...
export async function signInWithProvider(
  ...
  Result<{ user: User; token: string; expiresAt: Date; created: boolean; needsProfile: boolean }>
> {
  const { profile, provider } = input;
  const label = provider.charAt(0) + provider.slice(1).toLowerCase();
  if (!profile.emailVerified) {
    return fail(
      appError('provider_email_unverified', `Your ${label} e-mail is not verified.`, 403),
    );
  }
  ...
  if (!user) {
    ...
    if (byEmail) {
      ...
      if (input.referralCode)
        await deps.referrals.markAccepted(input.referralCode.toUpperCase(), user.id, now);
      await deps.log.record({
        userId: user.id,
        email: user.email,
        event: 'SIGNUP',

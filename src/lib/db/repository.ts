...
export class IndexedDbRepository implements Repository {
  ...
  async listSessions(): Promise<Session[]> {
    ...
    if (legacy.length) {
      await Promise.all(
        legacy.map((s) => this.db.sessions.update(s.id, { sourceFileName: s.name })),
      );
    ...
    const revive = <T extends object>(o: T, keys: (keyof T)[]): T => {
      for (const k of keys) {
        ...
        if (typeof v === 'string' || typeof v === 'number')
          (o as Record<string, unknown>)[k as string] = new Date(v);
      }
      return o;
    };
    const sessions = data.sessions.map((s) =>
      revive(s, ['importedAt', 'firstHandAt', 'lastHandAt']),
    );

...
export async function usesOfTag(tagId: string): Promise<TagUse[]> {
  ...
  const tagged = new Set(
    reviews.filter((review) => review.tags.includes(tagId)).map((review) => review.handId),
  );
  if (tagged.size === 0) return [];

  const uses: TagUse[] = [];
  for (const session of sessions) {
    const hits = session.handIds
      .map((id, index) => ({ id, index }))
      .filter((row) => tagged.has(row.id));
...
export async function removeTagEverywhere(tagId: string): Promise<number> {
  ...
  for (const review of affected) {
    await repo.saveReview({
      ...review,
      tags: review.tags.filter((id) => id !== tagId),
      updatedAt: new Date(),
    });
  }

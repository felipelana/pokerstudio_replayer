...
describe('built-in skins', () => {
  ...
  it('define every colour token and a chip colour per denomination', () => {
    for (const skin of BUILT_IN_SKINS) {
      for (const [k, v] of Object.entries(skin.ui))
        expect(isColor(v), `${skin.id}.ui.${k}`).toBe(true);
      ...
      for (const [k, v] of Object.entries(skin.deck.suitColors))
        expect(isColor(v), `${skin.id}.suit.${k}`).toBe(true);
      for (const d of CHIP_DENOMINATIONS)
        expect(skin.chips.colors[String(d)], `${skin.id}.chip.${d}`).toBeTruthy();
  ...
  it('deck presets never use more colours than their mode allows', () => {
    for (const p of DECK_PRESETS) {
      ...
      expect(new Set(Object.values(p.deck.suitColors)).size, p.id).toBeLessThanOrEqual(
        p.deck.colorMode,
      );
    }
  });

...
import { analyticsAllowed } from '@/components/ui/ConsentBanner';

export type UsageType =
  'APP_OPEN' | 'HAND_IMPORT' | 'REVIEW_START' | 'REPORT_EXPORT' | 'SKIN_APPLIED';
...
function anonId(): string | undefined {
  try {
    ...
    const fresh = (globalThis.crypto?.randomUUID?.() ?? `${Date.now()}${Math.random()}`).replace(
      /[^A-Za-z0-9]/g,
      '',
    );
...
export function track(
  type: UsageType,
  extra?: { skinId?: string; meta?: Record<string, string | number | boolean> },
) {

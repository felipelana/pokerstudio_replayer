...
import { CURRENT_RELEASE, hasUnread } from '@/lib/content/releaseNotes';
    ...
    >
        ...
        {overTable && (
          <>
            <button
              ...
              onClick={() =>
                updateSettings({ renderer: settings.renderer === 'svg' ? 'three' : 'svg' })
              }
            >
            ...
            <button
              ...
              style={
                settings.neon ? { color: 'var(--accent)', borderColor: 'var(--accent)' } : undefined
              }
      ...
      {inReplayer && (
        <button
          ...
          onClick={() =>
            updateSettings({ chipDisplay: settings.chipDisplay === 'bb' ? 'chips' : 'bb' })
          }
        >
          <span className={settings.chipDisplay === 'chips' ? 'font-bold' : 'opacity-50'}>
            {t('header.chips')}
          </span>
          <span className="opacity-40">/</span>
          <span className={settings.chipDisplay === 'bb' ? 'font-bold' : 'opacity-50'}>
            {t('header.bb')}
          </span>
        </button>
      )}

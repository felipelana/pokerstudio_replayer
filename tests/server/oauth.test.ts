...
import { signInWithProvider } from '../../src/server/application/auth/SignInWithProvider.js';
import { verifyIdToken } from '../../src/server/infrastructure/oauth/google.js';
import { signUpUser } from '../../src/server/application/auth/SignUpUser.js';
import { LANGUAGE_CODES } from '@pokerstudio/shared';
import type { OAuthProfile } from '../../src/server/application/ports/oauth.js';
import {
  fakeAttempts,
  fakeEmail,
  fakeGeo,
...
describe('Google identity token', () => {
  ...
  it('refuses a token issued for another application', async () => {
    ...
    await expect(verifyIdToken(token, CLIENT_ID, 'the-nonce', keyFor)).rejects.toThrow(
      /another application/,
    );
  });
  ...
  it('refuses a mismatched nonce, a wrong issuer and an expired token', async () => {
    await expect(
      verifyIdToken(makeIdToken(basePayload()), CLIENT_ID, 'other-nonce', keyFor),
    ).rejects.toThrow(/nonce/);
    await expect(
      verifyIdToken(
        makeIdToken(basePayload({ iss: 'https://evil.example' })),
        CLIENT_ID,
        'the-nonce',
        keyFor,
      ),
    ...
    await expect(
      verifyIdToken(
        makeIdToken(basePayload({ exp: Math.floor(Date.now() / 1000) - 10 })),
        CLIENT_ID,
        'the-nonce',
        keyFor,
      ),
...
describe('sign in with Google', () => {
  ...
  it('refuses a profile whose provider e-mail is not verified', async () => {
    const deps = googleDeps();
    const result = await signInWithProvider(deps, {
      provider: 'GOOGLE',
  ...
  it('keeps the referral code that survived the redirect', async () => {
    ...
    const result = await signInWithProvider(deps, {
      provider: 'GOOGLE',
      profile,
      referralCode: signup.value.user.referralCode,
    });

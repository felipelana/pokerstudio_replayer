...
import type { Session } from '../../src/server/domain/entities/Session.js';
import type {
  Captcha,
  Clock,
  EmailSender,
  Hasher,
  TokenGenerator,
} from '../../src/server/application/ports/index.js';
...
export function fakeTokens(): EmailTokenRepository & {
  all: { userId: string; type: string; tokenHash: string; expiresAt: Date; usedAt?: Date }[];
} {
  ...
  return {
    ...
    async consume(tokenHash, type, now) {
      const row = all.find(
        (t) => t.tokenHash === tokenHash && t.type === type && !t.usedAt && t.expiresAt > now,
      );
...
export function fakeErrors(): ErrorLogRepository & { entries: ErrorLogInput[] } {
  ...
  return {
    ...
    async summarise() {
      ...
      return [...counts].map(([level, count]) => ({
        level: level as ErrorLogInput['level'],
        count,
      }));
    },
    async purgeOlderThan() {
...
export function fakeAttempts(): LoginAttemptRepository & {
  state: Map<string, { count: number; lockedUntil?: Date }>;
} {
...
export function fakeEmail(): EmailSender & {
  sent: { to: string; template: string; payload: Record<string, unknown> }[];
} {
...
export const fakeUa = {
  parse: () => ({ deviceType: 'DESKTOP', os: 'Windows', browser: 'Chrome', browserVersion: '120' }),
};

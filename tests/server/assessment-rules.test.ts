...
const played = (assessment?: HandInput['assessment']): HandInput => ({
  heroVpip: true,
  assessment,
});
const folded = (assessment?: HandInput['assessment']): HandInput => ({
  heroVpip: false,
  assessment,
});
...
describe('the rubric', () => {
  ...
  it('refuses a gap, an overlap, or an edge that does not reach', () => {
    expect(
      validateRubric([
        { minScore: 0, maxScore: 40, description: 'a' },
        { minScore: 50, maxScore: 100, description: 'b' },
      ]),
    ).toContain('Faltam notas entre 40 e 50.');
    expect(
      validateRubric([
        { minScore: 0, maxScore: 60, description: 'a' },
        { minScore: 40, maxScore: 100, description: 'b' },
      ]).some((p) => p.includes('se sobrepõem')),
    ).toBe(true);
    expect(validateRubric([{ minScore: 10, maxScore: 100, description: 'a' }])).toContain(
      'A primeira faixa precisa começar em 0.',
    );
    expect(validateRubric([{ minScore: 0, maxScore: 90, description: 'a' }])).toContain(
      'A última faixa precisa terminar em 100.',
    );
  });
});
...
describe('coach passwords', () => {
  ...
  it('asks for eight characters, both cases and a digit', () => {
    ...
    expect(validateCoachPassword('Ab3xKp7')).toContain(
    ...
    expect(validateCoachPassword('Ab34Kpzz')).toContain(
      'Evite números em sequência, como 12 ou 21.',
    );
  });

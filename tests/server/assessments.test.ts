...
import {
  ...
  selfAssessment,
  sessionAssessments,
} from '../../src/server/application/assessments/Assessments.js';
import {
  createInvite,
  inviteState,
  listInvites,
  ...
  saveShareSettings,
  shareSettings,
} from '../../src/server/application/assessments/CoachInvites.js';
import { argon2Hasher } from '../../src/server/infrastructure/crypto/index.js';
...
async function inviteCoach(name: string, password = 'Ab3xKp7z') {
  const created = await createInvite(inviteDeps, userId, {
...
describe('the way in for a coach', () => {
  ...
  it('refuses a password that breaks the rule, naming the rule', async () => {
    const weak = await createInvite(inviteDeps, userId, {
      reviewSessionId: sessionId,
      coachName: 'Ana',
      password: 'ab34kpzz',
    });

...
import { appSecretProof } from '../../src/server/infrastructure/oauth/facebook.js';
import { verifyOidcToken } from '../../src/server/infrastructure/oauth/oidc.js';
...
function appleToken(payload: Record<string, unknown>, key = privateKey): string {
  const header = Buffer.from(JSON.stringify({ alg: 'ES256', kid: 'apple-key' })).toString(
    'base64url',
  );
...
describe('Apple identity token', () => {
  ...
  it('refuses another application, a wrong issuer, an expired token and a bad nonce', async () => {
    await expect(verify(appleToken(claims({ aud: 'com.someone.else' })))).rejects.toThrow(
      /another application/,
    );
    await expect(verify(appleToken(claims({ iss: 'https://evil.example' })))).rejects.toThrow(
      /issuer/,
    );
    await expect(
      verify(appleToken(claims({ exp: Math.floor(Date.now() / 1000) - 5 }))),
  ...
  it('refuses an unverified address, however it is spelled', async () => {
    await expect(verify(appleToken(claims({ email_verified: 'false' })))).rejects.toThrow(
      /unverified/,
    );
    await expect(verify(appleToken(claims({ email_verified: false })))).rejects.toThrow(
      /unverified/,
...
describe('Facebook access-token proof', () => {
  it('is the HMAC-SHA256 of the token under the app secret', () => {
    ...
    expect(proof).toBe(
      createHmac('sha256', 'the-app-secret').update('the-access-token').digest('hex'),
    );

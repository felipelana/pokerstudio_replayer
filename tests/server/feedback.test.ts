...
import {
  ...
  unreadFeedback,
  updateFeedback,
} from '../../src/server/application/feedback/Feedback.js';
...
describe('feedback', () => {
  ...
  it('trims what was typed, so stray whitespace never reaches the box', async () => {
    await submitFeedback(
      deps,
      userId,
      note({ subject: '  Spacing  ', body: '  The board spacing setting is buried.  ' }),
    );
  ...
  it('lets the team answer a note, and counts what is still unread', async () => {
    ...
    const updated = await updateFeedback(deps, created.value.id, {
      status: 'PLANNED',
      adminNote: 'Next release.',
    });
  ...
  it('filters the box by state, and says so when a note is gone', async () => {
    ...
    expect((await listFeedback(deps, { status: 'DONE' })).items.map((r) => r.subject)).toEqual([
      'One',
    ]);
    expect((await listFeedback(deps, { status: 'NEW' })).items.map((r) => r.subject)).toEqual([
      'Two',
    ]);

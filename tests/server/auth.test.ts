...
import { signUpUser } from '../../src/server/application/auth/SignUpUser.js';
import { loginUser, MAX_ATTEMPTS } from '../../src/server/application/auth/LoginUser.js';
import { verifyEmail } from '../../src/server/application/auth/VerifyEmail.js';
import {
  requestPasswordReset,
  resetPassword,
} from '../../src/server/application/auth/ResetPassword.js';
import { blockUser } from '../../src/server/application/admin/ManageUsers.js';
import {
  deleteUserSkin,
  listUserSkins,
  saveUserSkin,
} from '../../src/server/application/skins/UserSkins.js';
import {
  fakeAttempts,
  fakeEmail,
  fakeGeo,
...
describe('sign up', () => {
  ...
  it('refuses a duplicate e-mail, a short password and unaccepted terms', async () => {
    ...
    const weak = await signUpUser(deps, {
      ...validSignup,
      email: 'b@example.com',
      password: 'short',
    });
    ...
    const noTerms = await signUpUser(deps, {
      ...validSignup,
      email: 'c@example.com',
      acceptedTerms: false,
    });
...
describe('login', () => {
  ...
  it('answers the same for an unknown e-mail and a wrong password', async () => {
    const deps = makeDeps();
    await signUpUser(deps, validSignup);
    const unknown = await loginUser(deps, {
      email: 'nobody@example.com',
      password: 'whatever-long',
    });
    const wrong = await loginUser(deps, {
      email: 'player@example.com',
  ...
  it('locks the account after ten failures', async () => {
    ...
    const locked = await loginUser(deps, {
      email: 'player@example.com',
      password: validSignup.password,
      ip: '10.0.0.1',
    });
...
describe('password reset', () => {
  ...
  it('changes the password and drops every session', async () => {
    ...
    const done = await resetPassword(deps, {
    ...
    const withNew = await loginUser(deps, {
      email: 'player@example.com',
      password: 'another-strong-pass',
    });
...
describe('blocking a user', () => {
  it('revokes their sessions in the same step', async () => {
    ...
    await blockUser(deps, {
      adminId: 'admin',
      userId: signup.value.user.id,
      reason: 'chip dumping',
    });
...
describe('per-user skins', () => {
  it('saves, overwrites, lists and deletes a skin', async () => {
    ...
    const updated = await saveUserSkin(deps, {
      userId: 'u1',
      skinId: 'custom-1',
      ...skin,
      name: 'Renamed',
  ...
  it('refuses a skin payload that is too large', async () => {
    ...
    const result = await saveUserSkin(deps, {
      userId: 'u1',
      skinId: 'big',
      name: 'Big',
      data: huge,
